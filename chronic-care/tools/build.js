...
write('lessons.js',
  'CDC.lessonData={};\n'
  +MODULES.map(m=>m.lessons.map(l=>`CDC.lessonData[${JSON.stringify(l.id)}]=`+JSON.stringify({title:l.title,sub:l.sub,one:l.one,deep:l.deep,pit:l.pit,quiz:l.quiz,next:l.next})+';').join('\n')).join('\n')+'\n');
write('terms.js','CDC.terms='+JSON.stringify(TERMS,null,1)+';\n');
write('notices.js','CDC.notices='+JSON.stringify(notices,null,1)+';\n');

/* ---------- 校验术语引用(内容里 <a class="kw"> 数量) ---------- */
let kwN=0;
MODULES.forEach(m=>m.lessons.forEach(l=>['one','deep','pit'].forEach(k=>{ kwN+=(String(l[k]).match(/class="kw"/g)||[]).length; })));

/* ---------- 注入 index.html ---------- */
const idxPath=path.join(ROOT,'index.html');
let html=fs.readFileSync(idxPath,'utf8');
const inject='<!-- content:start -->\n<script src="content/modules.js"></script>\n<script src="content/lessons.js"></script>\n<script src="content/terms.js"></script>\n<script src="content/notices.js"></script>\n<!-- content:end -->';
