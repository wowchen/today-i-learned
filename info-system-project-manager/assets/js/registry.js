...
ISPM.registerLesson = function(data) {
  ISPM.lessons[data.id] = data;
  ISPM.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
ISPM.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < ISPM.searchIndex.length; i++) {
    var item = ISPM.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < ISPM.terms.length; t++) {
    var term = ISPM.terms[t];
    var ts = 0;
