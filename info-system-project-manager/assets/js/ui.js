...
ISPM.render = function(html) {
  document.getElementById('app').innerHTML = html;
  ISPM.bindTerms();
};

ISPM.findTerm = function(id) {
  for (var i = 0; i < ISPM.terms.length; i++) {
    if (ISPM.terms[i].id === id) return ISPM.terms[i];
  }
  return null;
};

ISPM.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      ISPM.showTermCard(id, this);
    });
  }
};

ISPM.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = ISPM.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + ISPM.esc(term.name) + '</b> <span class="tc-en">' + ISPM.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + ISPM.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + ISPM.esc(term.analogy) + '</div>' : '') +
