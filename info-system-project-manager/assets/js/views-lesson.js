...
ISPM.views.lesson = function(id) {
  var l = ISPM.lessons[id];
  if (!l) {
    ISPM.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + ISPM.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = ISPM.path.indexOf(id);
  ...
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? ISPM.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + ISPM.esc(l.title) + '</h1>';
...
ISPM.markAndNext = function(id) {
  var P = ISPM.progress();
  P.markRead(id);
  var idx = ISPM.path.indexOf(id);
  if (idx < ISPM.path.length - 1) {
    window.location.hash = '#/l/' + ISPM.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + ISPM.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + ISPM.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
