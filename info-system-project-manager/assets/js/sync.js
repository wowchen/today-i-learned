/* GitHub 进度同步(可选):fine-grained PAT + 独立 private repo + Contents API。
   ...
   注意:本站进度是工厂函数(ISPM.progress()),sync 内所有调用都带括号。 */
(function () {
  'use strict';
  window.ISPM = window.ISPM || {};

  var CFG_KEY = 'ispm.sync.v1';
  var cfg = loadCfg();
  var pushTimer = null;
  var busy = false;
  var lastSha = null;
  ...
  function apiUrl() {
    // cfg.repo 形如 "owner/progress"
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/ispm.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  function pull() {
    ...
    return fetch(apiUrl(), { headers: headers() }).then(function (res) {
      ...
      return res.json().then(function (j) {
        lastSha = j.sha;
        ISPM.progress().merge(JSON.parse(b64decodeUtf8(j.content)));
        return true;
      });
    });
  }
  ...
  function push(retry) {
    ...
    var body = {
      ...
      content: b64encodeUtf8(ISPM.progress().exportJson()),
  ...
  ISPM.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/ispm.json').trim();
      saveCfg();
    },
