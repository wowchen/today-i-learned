...
s += 'ISL.modules = ' + JSON.stringify(modObjs, null, 0) + ';\n';
s += 'ISL.path = ' + JSON.stringify(path_) + ';\n';
s += 'ISL.totalLessons = ' + totalLessons + ';\n';
w(C('modules.js'), s);

/* terms.js */
const termObjs = TERMS.map(t => ({ id: t[0], name: t[1], en: t[2], def: t[3], analogy: t[4] || '' }));
w(C('terms.js'), '/* 术语表(自动生成) */\nISL.terms = ' + JSON.stringify(termObjs) + ';\n');
