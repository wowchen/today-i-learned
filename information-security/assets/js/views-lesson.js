...
ISL.views.lesson = function(id) {
  var l = ISL.lessons[id];
  if (!l) {
    ISL.render('<div class="empty-state"><h2 class="page">课时待编写</h2>' +
      '<p>ID: ' + ISL.esc(id) + '</p><a class="btn" href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = ISL.path.indexOf(id);
  var mod = ISL.modules.find(function(m) { return m.id === l.module; });
  ...
  h += '<div class="pager">';
  if (pathIdx > 0) {
    var prev = ISL.path[pathIdx - 1];
    h += '<a href="#/l/' + prev + '">上一课<b>' + esc(ISL.lessons[prev] ? ISL.lessons[prev].title : prev) + '</b></a>';
  }
  if (!P.isRead(id)) {
  ...
  if (pathIdx >= 0 && pathIdx < ISL.path.length - 1) {
    ...
    h += '<a class="next" href="#/l/' + next + '">下一课<b>' + esc(ISL.lessons[next] ? ISL.lessons[next].title : next) + '</b></a>';
  }
  h += '</div>';
...
ISL.markAndNext = function(id) {
  var P = ISL.progress();
  P.markRead(id);
  var idx = ISL.path.indexOf(id);
  if (idx >= 0 && idx < ISL.path.length - 1) {
    window.location.hash = '#/l/' + ISL.path[idx + 1];
  } else {
    ISL.go('#/');
  }
