...
ISL.findTerm = function(id) {
  for (var i = 0; i < ISL.terms.length; i++) {
    if (ISL.terms[i].id === id) return ISL.terms[i];
  }
  return null;
};

ISL.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      ISL.showTermCard(id, this);
    });
  }
};

ISL.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = ISL.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + ISL.esc(term.name) + '</b> <span class="tc-en">' + ISL.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + ISL.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + ISL.esc(term.analogy) + '</div>' : '') +
