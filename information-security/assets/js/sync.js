/* GitHub 进度同步(可选):fine-grained PAT + 独立 private repo + Contents API。
   ...
   注意:本站进度是工厂函数(ISL.progress()),sync 内所有调用都带括号。 */
(function () {
  'use strict';
  window.ISL = window.ISL || {};

  var CFG_KEY = 'isl.sync.v1';
  var cfg = loadCfg();
  var pushTimer = null;
  var busy = false;
  var lastSha = null;
  ...
  function apiUrl() {
    ...
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/isl.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  function pull() {
    ...
    return fetch(apiUrl(), { headers: headers() }).then(function (res) {
      ...
      return res.json().then(function (j) {
        lastSha = j.sha;
        ISL.progress().merge(JSON.parse(b64decodeUtf8(j.content)));
        return true;
      });
    });
  }
  ...
  function push(retry) {
    ...
    var body = {
      ...
      content: b64encodeUtf8(ISL.progress().exportJson()),
  ...
  ISL.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/isl.json').trim();
      saveCfg();
    },
