...
FIN.registerLesson = function(data) {
  FIN.lessons[data.id] = data;
  FIN.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
FIN.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < FIN.searchIndex.length; i++) {
    var item = FIN.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < FIN.terms.length; t++) {
    var term = FIN.terms[t];
    var ts = 0;
