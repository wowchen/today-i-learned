...
FIN.render = function(html) {
  document.getElementById('app').innerHTML = html;
  FIN.bindTerms();
};

FIN.findTerm = function(id) {
  for (var i = 0; i < FIN.terms.length; i++) {
    if (FIN.terms[i].id === id) return FIN.terms[i];
  }
  return null;
};

FIN.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      FIN.showTermCard(id, this);
    });
  }
};

FIN.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = FIN.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + FIN.esc(term.name) + '</b> <span class="tc-en">' + FIN.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + FIN.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + FIN.esc(term.analogy) + '</div>' : '') +
