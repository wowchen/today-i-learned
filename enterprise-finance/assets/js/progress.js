...
(function() {
  ...
  function save(data) {
    ...
    if (FIN.sync) FIN.sync.schedulePush();
    if (FIN.onProgressChange) FIN.onProgressChange();
  }

  FIN.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < FIN.path.length; i++) {
          if (!this.isRead(FIN.path[i])) return i;
        }
        return FIN.path.length;
      },
