...
CHS.render = function(html) {
  document.getElementById('app').innerHTML = html;
  CHS.bindTerms();
};

CHS.findTerm = function(id) {
  for (var i = 0; i < CHS.terms.length; i++) {
    if (CHS.terms[i].id === id) return CHS.terms[i];
  }
  return null;
};

CHS.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      CHS.showTermCard(id, this);
    });
  }
};

CHS.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = CHS.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + CHS.esc(term.name) + '</b> <span class="tc-en">' + CHS.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + CHS.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + CHS.esc(term.analogy) + '</div>' : '') +
