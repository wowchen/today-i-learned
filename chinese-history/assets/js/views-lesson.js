...
CHS.views.lesson = function(id) {
  var l = CHS.lessons[id];
  if (!l) {
    CHS.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + CHS.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = CHS.path.indexOf(id);
  var stepNum = pathIdx >= 0 ? pathIdx + 1 : '';
  var mod = CHS.modules.find(function(m) { return m.id === l.module; });

  var html = '<article class="lesson">';
  html += '<div class="lesson-head">';
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? CHS.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + CHS.esc(l.title) + '</h1>';
  if (mod && (mod.era || mod.yearStart)) {
    var yrLabel = mod.yearStart ? mod.yearStart + (mod.yearEnd || '') : '';
    html += '<div class="lesson-era">';
    if (mod.era) html += '<b>' + CHS.esc(mod.era) + '</b>';
    if (yrLabel) html += CHS.esc(yrLabel);
    html += '</div>';
  }
  ...
  if (pathIdx > 0) {
    var prev = CHS.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="CHS.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="CHS.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < CHS.path.length - 1) {
    var next = CHS.path[pathIdx + 1];
...
CHS.markAndNext = function(id) {
  var P = CHS.progress();
  P.markRead(id);
  var idx = CHS.path.indexOf(id);
  if (idx < CHS.path.length - 1) {
    window.location.hash = '#/l/' + CHS.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + CHS.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + CHS.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
CHS.toggleRead = function(id) {
  var P = CHS.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  CHS.views.lesson(id);
  if (CHS.renderShell) CHS.renderShell();
};
