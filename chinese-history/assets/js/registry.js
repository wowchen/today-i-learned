...
CHS.registerLesson = function(data) {
  CHS.lessons[data.id] = data;
  CHS.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
CHS.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < CHS.searchIndex.length; i++) {
    var item = CHS.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < CHS.terms.length; t++) {
    var term = CHS.terms[t];
    var ts = 0;
