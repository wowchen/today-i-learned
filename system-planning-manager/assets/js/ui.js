...
SPM.render = function(html) {
  document.getElementById('app').innerHTML = html;
  SPM.bindTerms();
};

SPM.findTerm = function(id) {
  for (var i = 0; i < SPM.terms.length; i++) {
    if (SPM.terms[i].id === id) return SPM.terms[i];
  }
  return null;
};

SPM.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      SPM.showTermCard(id, this);
    });
  }
};

SPM.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = SPM.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + SPM.esc(term.name) + '</b> <span class="tc-en">' + SPM.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + SPM.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + SPM.esc(term.analogy) + '</div>' : '') +
