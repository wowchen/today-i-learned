...
SPM.views.lesson = function(id) {
  var l = SPM.lessons[id];
  if (!l) {
    SPM.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + SPM.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = SPM.path.indexOf(id);
  ...
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? SPM.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + SPM.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = SPM.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="SPM.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="SPM.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < SPM.path.length - 1) {
    var next = SPM.path[pathIdx + 1];
...
SPM.markAndNext = function(id) {
  var P = SPM.progress();
  P.markRead(id);
  var idx = SPM.path.indexOf(id);
  if (idx < SPM.path.length - 1) {
    window.location.hash = '#/l/' + SPM.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + SPM.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + SPM.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
SPM.toggleRead = function(id) {
  var P = SPM.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  SPM.views.lesson(id);
  if (SPM.renderShell) SPM.renderShell();
};
