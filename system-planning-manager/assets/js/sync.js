/* GitHub 进度同步(可选):fine-grained PAT + 独立 private 仓库 + Contents API。
...
(function () {
  ...
  function apiUrl() {
    // cfg.repo 形如 "owner/spm-progress"
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/spm.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  function pull() {
    ...
    return fetch(apiUrl(), { headers: headers() }).then(function (res) {
      ...
      return res.json().then(function (j) {
        lastSha = j.sha;
        SPM.progress().merge(JSON.parse(b64decodeUtf8(j.content)));
        return true;
      });
    });
  }
  ...
  function push(retry) {
    ...
    var body = {
      ...
      content: b64encodeUtf8(SPM.progress().exportJson()),
  ...
  SPM.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/spm.json').trim();
      saveCfg();
    },
