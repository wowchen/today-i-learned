...
(function() {
  ...
  function save(data) {
    ...
    if (ML.sync) ML.sync.schedulePush();
    if (ML.onProgressChange) ML.onProgressChange();
  }

  ML.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < ML.path.length; i++) {
          if (!this.isRead(ML.path[i])) return i;
        }
        return ML.path.length;
      },
