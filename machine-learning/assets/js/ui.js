...
ML.render = function(html) {
  document.getElementById('app').innerHTML = html;
  ML.bindTerms();
};

ML.findTerm = function(id) {
  for (var i = 0; i < ML.terms.length; i++) {
    if (ML.terms[i].id === id) return ML.terms[i];
  }
  return null;
};

ML.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      ML.showTermCard(id, this);
    });
  }
};

ML.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = ML.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + ML.esc(term.name) + '</b> <span class="tc-en">' + ML.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + ML.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + ML.esc(term.analogy) + '</div>' : '') +
