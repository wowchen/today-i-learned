...
ML.views.lesson = function(id) {
  var l = ML.lessons[id];
  if (!l) {
    ML.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + ML.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = ML.path.indexOf(id);
  var stepNum = pathIdx >= 0 ? pathIdx + 1 : '';
  var mod = ML.modules.find(function(m) { return m.id === l.module; });

  var html = '<article class="lesson">';
  html += '<div class="lesson-head">';
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? ML.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + ML.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = ML.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="ML.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="ML.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < ML.path.length - 1) {
    var next = ML.path[pathIdx + 1];
...
ML.markAndNext = function(id) {
  var P = ML.progress();
  P.markRead(id);
  var idx = ML.path.indexOf(id);
  if (idx < ML.path.length - 1) {
    window.location.hash = '#/l/' + ML.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + ML.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + ML.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
ML.toggleRead = function(id) {
  var P = ML.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  ML.views.lesson(id);
  if (ML.renderShell) ML.renderShell();
};
