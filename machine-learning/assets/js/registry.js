...
ML.registerLesson = function(data) {
  ML.lessons[data.id] = data;
  ML.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
...
ML.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < ML.searchIndex.length; i++) {
    var item = ML.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < ML.terms.length; t++) {
    var term = ML.terms[t];
    var ts = 0;
