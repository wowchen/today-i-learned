...
SAN.registerLesson = function(data) {
  SAN.lessons[data.id] = data;
  SAN.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
SAN.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < SAN.searchIndex.length; i++) {
    var item = SAN.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < SAN.terms.length; t++) {
    var term = SAN.terms[t];
    var ts = 0;
