...
SAN.views.home = function() {
  var P = SAN.progress();
  var step = P.currentStep();
  var readCount = P.readCount();
  var total = SAN.totalLessons;
  var percent = total > 0 ? Math.round(readCount / total * 100) : 0;
  var streak = P.streak();
  var termCount = P.termCount();

  var nextLesson = step < SAN.path.length ? SAN.path[step] : null;
  var nextTitle = '';
  var nextModule = '';
  if (nextLesson) {
    var l = SAN.lessons[nextLesson];
    if (l) {
      nextTitle = l.title;
      ...
      var mod = SAN.modules.find(function(m) { return m.id === parts[0]; });
  ...
  if (nextLesson) {
    ...
    html += '<p>' + SAN.esc(nextModule) + ' › ' + SAN.esc(nextTitle) + '</p>';
    if (SAN.lessons[nextLesson]) {
  ...
  for (var i = 0; i < SAN.modules.length; i++) {
    var m = SAN.modules[i];
    var mRead = 0;
    for (var j = 0; j < SAN.path.length; j++) {
      if (SAN.path[j].indexOf(m.id + '/') === 0 && P.isRead(SAN.path[j])) mRead++;
    }
    html += '<a class="module-card" href="#/m/' + m.id + '">';
    html += '<span class="tag tag-' + tagClass(m.tag) + '">' + SAN.esc(m.tag) + '</span>';
    html += '<h4>' + SAN.esc(m.title) + '</h4>';
    html += '<p>' + SAN.esc(m.desc) + '</p>';
