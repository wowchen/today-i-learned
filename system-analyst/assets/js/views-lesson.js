...
SAN.views.lesson = function(id) {
  var l = SAN.lessons[id];
  if (!l) {
    SAN.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + SAN.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = SAN.path.indexOf(id);
  var stepNum = pathIdx >= 0 ? pathIdx + 1 : '';
  var mod = SAN.modules.find(function(m) { return m.id === l.module; });

  var html = '<article class="lesson">';
  html += '<div class="lesson-head">';
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? SAN.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + SAN.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = SAN.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="SAN.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="SAN.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < SAN.path.length - 1) {
    var next = SAN.path[pathIdx + 1];
...
SAN.markAndNext = function(id) {
  var P = SAN.progress();
  P.markRead(id);
  var idx = SAN.path.indexOf(id);
  if (idx < SAN.path.length - 1) {
    window.location.hash = '#/l/' + SAN.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + SAN.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + SAN.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
SAN.toggleRead = function(id) {
  var P = SAN.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  SAN.views.lesson(id);
  if (SAN.renderShell) SAN.renderShell();
};
