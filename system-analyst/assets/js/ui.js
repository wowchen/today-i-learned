...
SAN.render = function(html) {
  document.getElementById('app').innerHTML = html;
  SAN.bindTerms();
};

SAN.findTerm = function(id) {
  for (var i = 0; i < SAN.terms.length; i++) {
    if (SAN.terms[i].id === id) return SAN.terms[i];
  }
  return null;
};

SAN.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      SAN.showTermCard(id, this);
    });
  }
};

SAN.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = SAN.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + SAN.esc(term.name) + '</b> <span class="tc-en">' + SAN.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + SAN.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + SAN.esc(term.analogy) + '</div>' : '') +
