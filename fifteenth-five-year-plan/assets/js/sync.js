/* GitHub 进度同步(ADR-4):fine-grained PAT + 独立 private repo + Contents API。
...
(function () {
  ...
  var CFG_KEY = 'ffp.sync.v1';
  var cfg = loadCfg();
  /* 迁移:早期版本默认路径为裸 progress.json(落在仓库根),
     现统一为 progress/fyp.json,避免多站同步到同一仓库时互相覆盖。 */
  function migrateLegacyPath() {
    if (cfg.path === 'progress.json') { cfg.path = 'progress/fyp.json'; saveCfg(); }
  }
  migrateLegacyPath();
  var pushTimer = null;
  var busy = false;
  ...
  function apiUrl() {
    ...
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/fyp.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  function pull() {
    ...
    return fetch(apiUrl(), { headers: headers() }).then(function (res) {
      ...
      return res.json().then(function (j) {
        lastSha = j.sha;
        FYP.progress.merge(JSON.parse(b64decodeUtf8(j.content)));
        return true;
      });
    });
  }
  ...
  function push(retry) {
    ...
    var body = {
      ...
      content: b64encodeUtf8(FYP.progress.exportJson()),
  ...
  FYP.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/fyp.json').trim();
      saveCfg();
    },
