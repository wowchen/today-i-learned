...
(function () {
  ...
  /* ── 首页 ── */
  FYP.views.home = function () {
    var next = FYP.nextStep();
    var hero;
    if (next) {
      ...
      hero =
        '<section class="hero">' +
        '<div class="main">' +
        '<p class="eyebrow"><b>第 ' + (next.index + 1) + ' 步</b>' + FYP.esc(mod.title) + '</p>' +
        '<h1><mark>' + FYP.esc(next.step.title) + '</mark></h1>' +
        '<p class="intro">约 ' + (R().lesson(next.step.id).minutes || 5) +
        ' 分钟一课。不用想"接下来学什么",跟着路线走就行。</p>' +
        '<div class="cta">' +
        '<a class="btn solid" href="#/l/' + next.step.id + '">继续学习 第 ' + (next.index + 1) + ' 步 ' + FYP.icon('arrowR', 13) + '</a>' +
    ...
    var demo =
      ...
      '读懂十五五,先认识几个词:它是一份<gd data-term="wnghj">五年规划</gd>;' +
      '里面有"必须做到"的<gd data-term="ysxzb">约束性指标</gd>;' +
      '靠科技发展<gd data-term="xzscl">新质生产力</gd>、用<gd data-term="rgzn">人工智能+</gd>改造产业;' +
      '还要稳步推进<gd data-term="tdf">碳达峰</gd>。' +
    ...
    FYP.render(hero + demo + toc + review);
  };

  function sidenote() {
    return '<aside class="sidenote">' +
      '<p><b>' + P().streak() + ' 天</b>连续学习</p>' +
      '<p><b>' + FYP.pathDone() + ' / ' + R().path().length + '</b>路线进度</p>' +
  ...
  function tocRow(m) {
    ...
    return '<li><span class="no">' + m.no + '</span>' +
      '<a class="t" href="#/m/' + m.id + '">' + FYP.esc(m.title) +
      '<small>' + FYP.esc(m.sub) + '</small></a>' +
      '<span class="lead"></span>' + pg + '</li>';
  }

  /* ── 完整路线 ── */
  FYP.views.path = function () {
    var next = FYP.nextStep();
    ...
    FYP.render(
      ...
      '<p class="sub">已完成 ' + FYP.pathDone() + ' / ' + R().path().length +
  ...
  FYP.views.module = function (id) {
    ...
    var coming = R().path().filter(function (s) {
      ...
      return '<li><span class="no">·</span><span class="t dead">' +
        FYP.esc(s.title) + '</span><span class="lead"></span><span class="pg">待编写</span></li>';
    }).join('');
    FYP.render(
      '<div class="crumb" style="margin-top:14px"><b>模块 ' + m.no + '</b> / ' + FYP.esc(m.sub) + '</div>' +
      '<h1 class="page">' + FYP.esc(m.title) + '</h1>' +
      '<p class="meta" style="font-family:inherit;font-size:14px;max-width:44em">' + FYP.esc(m.desc) + '</p>' +
