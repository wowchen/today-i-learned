...
(function () {
  ...
  function save() {
    ...
    if (FYP.sync) FYP.sync.schedulePush();
    if (FYP.onProgressChange) FYP.onProgressChange();
  }
  ...
  FYP.progress = {
    ...
    readCountOf: function (moduleId) {
      var n = 0;
      FYP.registry.lessonsOf(moduleId).forEach(function (l) {
        if (FYP.progress.isRead(l.id)) n++;
      });
      return n;
    },
