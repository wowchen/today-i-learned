...
(function () {
  ...
  function lookup(g) {
    ...
    if (id) {
      var t = FYP.registry.term(id);
      if (t) return t;
    }
    return FYP.registry.termByName(g.textContent.trim());
  }
  ...
  function show(g) {
    ...
    if (!t) {
      ...
      p.innerHTML = '<div class="t">' + FYP.esc(g.textContent.trim()) + '</div>' +
        '<div class="a">这个术语的卡片还没编写。</div>';
    } else {
      var faved = FYP.progress.hasTerm(t.id);
      var lessonLink = '';
      if (t.lesson && FYP.registry.lesson(t.lesson)) {
        lessonLink = '<a href="#/l/' + t.lesson + '">去那一课 ' + FYP.icon('arrowR', 12) + '</a>';
      }
      p.innerHTML =
        '<div class="t">' + FYP.esc(t.name) +
        (t.en ? '<small>' + FYP.esc(t.en) + '</small>' : '') + '</div>' +
        '<div class="d">' + FYP.esc(t.def) + '</div>' +
        (t.analogy ? '<div class="a">类比:' + FYP.esc(t.analogy) + '</div>' : '') +
        '<div class="ops">' +
        (faved
          ? '<button type="button" class="faved">已在术语本 ✓</button>'
          : '<button type="button" class="fav">' + FYP.icon('star', 12) + ' 收藏</button>') +
        lessonLink + '</div>';
      var favBtn = p.querySelector('.fav');
      if (favBtn) favBtn.onclick = function (ev) {
        ev.stopPropagation();
        FYP.progress.addTerm(t.id, FYP.currentLessonId);
