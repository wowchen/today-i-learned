...
(function () {
  'use strict';
  var R = function () { return FYP.registry; };
  var P = function () { return FYP.progress; };

  /* ── 术语表:全站术语,按模块分组 ── */
  FYP.views.terms = function () {
    var all = R().terms();
    var byModule = {};
    ...
    var groups = R().modules().filter(function (m) { return byModule[m.id]; })
      .map(function (m) {
        return '<div class="glossary-group">' +
          '<div class="chapter"><span>模块 ' + m.no + '</span><h2>' + FYP.esc(m.title) + '</h2></div>' +
    ...
    input.addEventListener('input', function () {
      ...
      t = setTimeout(function () {
        ...
        box.innerHTML = hits.length
          ...
          : '<p class="empty">没有找到「' + FYP.esc(q) + '」。</p>';
      }, 180);
    });
  };
  ...
  function termRow(t) {
    var src = t.lesson && R().lesson(t.lesson)
      ? '<a class="src" href="#/l/' + t.lesson + '">出自:' + FYP.esc(R().lesson(t.lesson).title) + '</a>' : '';
    return '<li><span class="term" data-termpop data-term="' + FYP.esc(t.id) + '">' + FYP.esc(t.name) + '</span>' +
      '<span class="def">' + FYP.esc(t.def) + '</span>' + src + '</li>';
  }

  /* ── 我的术语本 ── */
  FYP.views.book = function () {
    var favs = P().listTerms();
    var list = favs.length ? '<ul class="terms">' + favs.map(function (f) {
      var t = R().term(f._id);
      if (!t) return '';
      var src = f.lesson && R().lesson(f.lesson)
        ? '<a class="src" href="#/l/' + f.lesson + '">出自:' + FYP.esc(R().lesson(f.lesson).title) + '</a>' : '';
      return '<li><span class="term" data-termpop data-term="' + FYP.esc(t.id) + '">' + FYP.esc(t.name) + '</span>' +
        '<span class="def">' + FYP.esc(t.def) + '</span>' + src +
        '<button class="del" data-del="' + FYP.esc(t.id) + '">删除</button></li>';
    }).join('') + '</ul>'
    ...
    FYP.render(
      ...
      (favs.length ? '<div style="margin-top:26px"><a class="btn ghost" href="#/review">用它们翻牌复习 ' + FYP.icon('arrowR', 12) + '</a></div>' : '')
    );
  ...
  /* ── 翻牌复习:随机抽 5 张,纯回顾不判分 ── */
  FYP.views.review = function () {
    var pool = P().listTerms().map(function (f) { return R().term(f._id); })
      .filter(function (t) { return !!t; });
    if (!pool.length) {
      FYP.render('<p class="empty">术语本还是空的,先去课程里收藏几个术语吧。<br><br><a class="btn solid" href="#/">去学习</a></p>');
      return;
    }
    ...
    FYP.render(
      ...
      '<button class="btn solid" id="nextBtn">下一张 ' + FYP.icon('arrowR', 12) + '</button>' +
    ...
    function draw() {
      ...
      if (!flipped) {
        face.innerHTML = '<p class="big">' + FYP.esc(c.name) + '</p>' +
          (c.en ? '<p class="small">' + FYP.esc(c.en) + '</p>' : '') +
          '<p class="small">这个术语什么意思?想好再翻面</p>';
      } else {
        face.innerHTML = '<p style="font-size:16.5px;max-width:34em">' + FYP.esc(c.def) + '</p>' +
          (c.analogy ? '<p class="small">类比:' + FYP.esc(c.analogy) + '</p>' : '') +
          (lessonTitle ? '<p class="small">出自:' + FYP.esc(lessonTitle) + '</p>' : '');
      }
    ...
    document.getElementById('reBtn').addEventListener('click', FYP.views.review);
    draw();
  };

  /* ── 搜索 ── */
  FYP.views.search = function () {
    FYP.render(
        ...
        if (hits.terms.length) {
          html += '<div class="chapter"><span>术语</span><h2>' + hits.terms.length + ' 条</h2></div>' +
            '<ul class="terms">' + hits.terms.map(function (tm) {
              return '<li><span class="term" data-termpop data-term="' + FYP.esc(tm.id) + '">' +
                FYP.esc(tm.name) + '</span><span class="def">' + FYP.esc(tm.def) + '</span></li>';
            }).join('') + '</ul>';
        }
        if (hits.lessons.length) {
          html += '<div class="chapter"><span>课程</span><h2>' + hits.lessons.length + ' 课</h2></div>' +
            hits.lessons.map(function (h) {
              ...
              return '<div class="result"><p class="crumb">' + FYP.esc(mod.title) + '</p>' +
                '<a href="#/l/' + h.lesson.id + '">' + FYP.esc(h.lesson.title) + '</a>' +
                '<p>' + FYP.esc(h.snippet) + '</p></div>';
            }).join('');
        }
        box.innerHTML = html || '<p class="empty">没有找到「' + FYP.esc(q) + '」——可能这部分内容还没编写。</p>';
      }, 200);
    });
  };

  /* ── 设置 ── */
  FYP.views.settings = function () {
    var cfg = FYP.sync.config();
    FYP.render(
      ...
      '<div class="set-row"><label>仓库</label><input id="syRepo" placeholder="owner/ffp-progress" value="' + FYP.esc(cfg.repo || '') + '"></div>' +
      '<div class="set-row"><label>分支</label><input id="syBranch" placeholder="main" value="' + FYP.esc(cfg.branch || 'main') + '"></div>' +
      '<div class="set-row"><label>Token</label><input id="syToken" type="password" placeholder="github_pat_…" value="' + FYP.esc(cfg.token || '') + '"></div>' +
      ...
      '<p class="sync-msg" id="syMsg">' + FYP.esc(FYP.sync.statusText) + '</p></div>' +

      '<div class="card set-group"><h3>手动备份</h3>' +
      '<p class="d">自动同步之外的备用通道:导出/导入 fyp-progress.json。导入采用智能合并,不会覆盖丢数据。</p>' +
      '<div class="set-row"><button class="btn" id="expBtn">导出 fyp-progress.json</button>' +
    ...
    document.getElementById('sySave').addEventListener('click', function () {
      ...
      if (!FYP.sync.ready()) { msg('仓库和 token 都要填。', 'bad'); return; }
      msg('同步中…');
      FYP.sync.pullNow().then(function () { return FYP.sync.pushNow(); })
        .then(function (ok) { msg(ok ? '已同步 ✓ 两台设备现在看到同一份进度。' : '同步失败:' + FYP.sync.statusText, ok ? 'ok' : 'bad'); });
    });
    document.getElementById('syPull').addEventListener('click', function () {
      msg('拉取中…');
      FYP.sync.pullNow().then(function (ok) { msg(ok ? '已拉取并合并远端进度 ✓' : FYP.sync.statusText, ok ? 'ok' : ''); });
    });
    ...
    document.getElementById('impFile').addEventListener('change', function (e) {
      ...
      fr.onload = function () {
        alert(P().importJson(fr.result) ? '导入成功,已与本机数据合并。' : '导入失败:不是有效的 fyp-progress.json。');
      };
      fr.readAsText(f);
    });
