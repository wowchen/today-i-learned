...
(function () {
  ...
  FYP.views.lesson = function (id) {
    var l = R().lesson(id);
    if (!l) {
      FYP.render('<p class="empty">这一课还在编写中,先看看<a href="#/path">路线上的其他课</a>。</p>');
      return;
    }
    FYP.currentLessonId = id;
    ...
    var head =
      ...
      (pi >= 0 ? '<b>第 ' + (pi + 1) + ' 步</b> / ' : '') +
      '模块 ' + mod.no + ' · ' + FYP.esc(mod.title) +
      ' / 第 ' + (l.order || '?') + ' 课</div>' +
      '<h1 class="page">' + FYP.esc(l.title) + '</h1>' +
      '<p class="meta">约 ' + (l.minutes || 5) + ' 分钟' +
      (l.key ? ' · <span class="key">' + FYP.esc(l.key) + '</span>' : '') +
      ' · 虚线术语可点查可收藏</p>';
  ...
  function pathNav(pi) {
    ...
    if (prev && R().lesson(prev.id)) out.push('<a href="#/l/' + prev.id + '">' + FYP.icon('arrowL', 12) + ' 上一课</a>');
    out.push('<a href="#/path">路线</a>');
    if (next) {
      out.push(R().lesson(next.id)
        ? '<a href="#/l/' + next.id + '">下一课 ' + FYP.icon('arrowR', 12) + '</a>'
