...
EMT.render = function(html) {
  document.getElementById('app').innerHTML = html;
  EMT.bindTerms();
};

EMT.findTerm = function(id) {
  for (var i = 0; i < EMT.terms.length; i++) {
    if (EMT.terms[i].id === id) return EMT.terms[i];
  }
  return null;
};

EMT.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      EMT.showTermCard(id, this);
    });
  }
};

EMT.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = EMT.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + EMT.esc(term.name) + '</b> <span class="tc-en">' + EMT.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + EMT.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + EMT.esc(term.analogy) + '</div>' : '') +
