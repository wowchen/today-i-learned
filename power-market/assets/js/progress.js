...
(function() {
  ...
  function save(data) {
    ...
    if (EMT.sync) EMT.sync.schedulePush();
    if (EMT.onProgressChange) EMT.onProgressChange();
  }

  EMT.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < EMT.path.length; i++) {
          if (!this.isRead(EMT.path[i])) return i;
        }
        return EMT.path.length;
      },
