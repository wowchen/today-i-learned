...
DAT.views.lesson = function(id) {
  var l = DAT.lessons[id];
  if (!l) {
    DAT.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + DAT.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = DAT.path.indexOf(id);
  var stepNum = pathIdx >= 0 ? pathIdx + 1 : '';
  var mod = DAT.modules.find(function(m) { return m.id === l.module; });

  var html = '<article class="lesson">';
  html += '<div class="lesson-head">';
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? DAT.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + DAT.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = DAT.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="DAT.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="DAT.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < DAT.path.length - 1) {
    var next = DAT.path[pathIdx + 1];
...
DAT.markAndNext = function(id) {
  var P = DAT.progress();
  P.markRead(id);
  var idx = DAT.path.indexOf(id);
  if (idx < DAT.path.length - 1) {
    window.location.hash = '#/l/' + DAT.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + DAT.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + DAT.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
DAT.toggleRead = function(id) {
  var P = DAT.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  DAT.views.lesson(id);
  if (DAT.renderShell) DAT.renderShell();
};
