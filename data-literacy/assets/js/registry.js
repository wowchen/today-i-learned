...
DAT.registerLesson = function(data) {
  DAT.lessons[data.id] = data;
  DAT.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
DAT.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < DAT.searchIndex.length; i++) {
    var item = DAT.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < DAT.terms.length; t++) {
    var term = DAT.terms[t];
    var ts = 0;
