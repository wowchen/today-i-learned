...
(function() {
  ...
  function save(data) {
    ...
    if (DAT.sync) DAT.sync.schedulePush();
    if (DAT.onProgressChange) DAT.onProgressChange();
  }

  DAT.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < DAT.path.length; i++) {
          if (!this.isRead(DAT.path[i])) return i;
        }
        return DAT.path.length;
      },
