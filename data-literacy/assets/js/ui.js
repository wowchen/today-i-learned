...
DAT.render = function(html) {
  document.getElementById('app').innerHTML = html;
  DAT.bindTerms();
};

DAT.findTerm = function(id) {
  for (var i = 0; i < DAT.terms.length; i++) {
    if (DAT.terms[i].id === id) return DAT.terms[i];
  }
  return null;
};

DAT.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      DAT.showTermCard(id, this);
    });
  }
};

DAT.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = DAT.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + DAT.esc(term.name) + '</b> <span class="tc-en">' + DAT.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + DAT.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + DAT.esc(term.analogy) + '</div>' : '') +
