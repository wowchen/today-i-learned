...
(function () {
  ...
  PGF.views.lesson = function (id) {
    var l = R().lesson(id);
    if (!l) {
      PGF.render('<p class="empty">这一课还在编写中,先看看<a href="#/path">路线上的其他课</a>。</p>');
      return;
    }
    PGF.currentLessonId = id;
    ...
    var head =
      ...
      (pi >= 0 ? '<b>第 ' + (pi + 1) + ' 步</b> / ' : '') +
      '模块 ' + mod.no + ' · ' + PGF.esc(mod.title) +
      ' / 第 ' + (l.order || '?') + ' 课</div>' +
      '<h1 class="page">' + PGF.esc(l.title) + '</h1>' +
      '<p class="meta">约 ' + (l.minutes || 5) + ' 分钟' +
      (l.key ? ' · <span class="key">' + PGF.esc(l.key) + '</span>' : '') +
      ' · 虚线术语可点查可收藏</p>';
  ...
  function pathNav(pi) {
    ...
    if (prev && R().lesson(prev.id)) out.push('<a href="#/l/' + prev.id + '">' + PGF.icon('arrowL', 12) + ' 上一课</a>');
    out.push('<a href="#/path">路线</a>');
    if (next) {
      out.push(R().lesson(next.id)
        ? '<a href="#/l/' + next.id + '">下一课 ' + PGF.icon('arrowR', 12) + '</a>'
