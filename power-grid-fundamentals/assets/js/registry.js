/* 内容注册中心:内容即数据(ADR-3)。
   ...
   PGF.registerModule / PGF.registerPath / PGF.registerLesson / PGF.registerTerms
...
(function () {
  ...
  PGF.registerModule = function (m) {
    if (!m || !m.id) return;
    if (!modules[m.id]) moduleOrder.push(m.id);
    modules[m.id] = m;
  };

  PGF.registerPath = function (list) { path = list || []; };

  PGF.registerLesson = function (l) {
    if (!l || !l.id || !l.module) return;
    lessons[l.id] = l;
  };

  /* 术语库:数组批量注册,可多次调用(分模块扩充) */
  PGF.registerTerms = function (list) {
