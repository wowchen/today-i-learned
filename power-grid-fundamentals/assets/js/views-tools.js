...
(function () {
  ...
  PGF.views.settings = function () {
    ...
    PGF.render(
      ...
      '<p class="d">自动同步之外的备用通道:导出/导入 pgf-progress.json。导入采用智能合并,不会覆盖丢数据。</p>' +
      '<div class="set-row"><button class="btn" id="expBtn">导出 pgf-progress.json</button>' +
    ...
    document.getElementById('impFile').addEventListener('change', function (e) {
      ...
      fr.onload = function () {
        alert(P().importJson(fr.result) ? '导入成功,已与本机数据合并。' : '导入失败:不是有效的 pgf-progress.json。');
      };
      fr.readAsText(f);
    });
