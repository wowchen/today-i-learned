...
(function () {
  ...
  function lookup(g) {
    ...
    if (id) {
      var t = PGF.registry.term(id);
      if (t) return t;
    }
    return PGF.registry.termByName(g.textContent.trim());
  }
  ...
  function show(g) {
    ...
    if (!t) {
      ...
      p.innerHTML = '<div class="t">' + PGF.esc(g.textContent.trim()) + '</div>' +
        '<div class="a">这个术语的卡片还没编写。</div>';
    } else {
      var faved = PGF.progress.hasTerm(t.id);
      var lessonLink = '';
      if (t.lesson && PGF.registry.lesson(t.lesson)) {
        lessonLink = '<a href="#/l/' + t.lesson + '">去那一课 ' + PGF.icon('arrowR', 12) + '</a>';
      }
      p.innerHTML =
        '<div class="t">' + PGF.esc(t.name) +
        (t.en ? '<small>' + PGF.esc(t.en) + '</small>' : '') + '</div>' +
        '<div class="d">' + PGF.esc(t.def) + '</div>' +
        (t.analogy ? '<div class="a">类比:' + PGF.esc(t.analogy) + '</div>' : '') +
        '<div class="ops">' +
        (faved
          ? '<button type="button" class="faved">已在术语本 ✓</button>'
          : '<button type="button" class="fav">' + PGF.icon('star', 12) + ' 收藏</button>') +
        lessonLink + '</div>';
      var favBtn = p.querySelector('.fav');
      if (favBtn) favBtn.onclick = function (ev) {
        ev.stopPropagation();
        PGF.progress.addTerm(t.id, PGF.currentLessonId);
