...
(function () {
  ...
  /* 迁移:早期版本默认路径为裸 progress.json(落在仓库根),
     现统一为 progress/ems.json,避免多站同步到同一仓库时互相覆盖。 */
  function migrateLegacyPath() {
    if (cfg.path === 'progress.json') { cfg.path = 'progress/ems.json'; saveCfg(); }
  }
  migrateLegacyPath();
  var pushTimer = null;
  var busy = false;
  ...
  function apiUrl() {
    ...
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/ems.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  EMS.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/ems.json').trim();
      saveCfg();
    },
