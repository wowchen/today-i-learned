...
GEO.render = function(html) {
  document.getElementById('app').innerHTML = html;
  GEO.bindTerms();
};

GEO.findTerm = function(id) {
  for (var i = 0; i < GEO.terms.length; i++) {
    if (GEO.terms[i].id === id) return GEO.terms[i];
  }
  return null;
};

GEO.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      GEO.showTermCard(id, this);
    });
  }
};

GEO.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = GEO.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + GEO.esc(term.name) + '</b> <span class="tc-en">' + GEO.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + GEO.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + GEO.esc(term.analogy) + '</div>' : '') +
