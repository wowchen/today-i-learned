...
(function () {
  ...
  function apiUrl() {
    ...
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/geo.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  function pull() {
    ...
    return fetch(apiUrl(), { headers: headers() }).then(function (res) {
      ...
      return res.json().then(function (j) {
        lastSha = j.sha;
        GEO.progress().merge(JSON.parse(b64decodeUtf8(j.content)));
        return true;
      });
    });
  }
  ...
  function push(retry) {
    ...
    var body = {
      ...
      content: b64encodeUtf8(GEO.progress().exportJson()),
  ...
  GEO.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/geo.json').trim();
      saveCfg();
    },
