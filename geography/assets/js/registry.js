...
GEO.registerLesson = function(data) {
  GEO.lessons[data.id] = data;
  GEO.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
GEO.search = function(query) {
  ...
  for (var i = 0; i < GEO.searchIndex.length; i++) {
    var item = GEO.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < GEO.terms.length; t++) {
    var term = GEO.terms[t];
    var ts = 0;
