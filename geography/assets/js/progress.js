...
(function() {
  ...
  function save(data) {
    ...
    if (GEO.sync) GEO.sync.schedulePush();
    if (GEO.onProgressChange) GEO.onProgressChange();
  }

  GEO.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < GEO.path.length; i++) {
          if (!this.isRead(GEO.path[i])) return i;
        }
        return GEO.path.length;
      },
