...
(function () {
  ...
  function apiUrl() {
    // cfg.repo 形如 "owner/math-progress"
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/math.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  function pull() {
    ...
    return fetch(apiUrl(), { headers: headers() }).then(function (res) {
      ...
      return res.json().then(function (j) {
        lastSha = j.sha;
        MATH.progress().merge(JSON.parse(b64decodeUtf8(j.content)));
        return true;
      });
    });
  }
  ...
  function push(retry) {
    ...
    var body = {
      ...
      content: b64encodeUtf8(MATH.progress().exportJson()),
  ...
  MATH.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/math.json').trim();
      saveCfg();
    },
