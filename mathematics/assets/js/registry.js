...
MATH.registerLesson = function(data) {
  MATH.lessons[data.id] = data;
  MATH.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
MATH.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < MATH.searchIndex.length; i++) {
    var item = MATH.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < MATH.terms.length; t++) {
    var term = MATH.terms[t];
    var ts = 0;
