...
MATH.render = function(html) {
  document.getElementById('app').innerHTML = html;
  MATH.bindTerms();
};

MATH.findTerm = function(id) {
  for (var i = 0; i < MATH.terms.length; i++) {
    if (MATH.terms[i].id === id) return MATH.terms[i];
  }
  return null;
};

MATH.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      MATH.showTermCard(id, this);
    });
  }
};

MATH.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = MATH.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + MATH.esc(term.name) + '</b> <span class="tc-en">' + MATH.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + MATH.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + MATH.esc(term.analogy) + '</div>' : '') +
