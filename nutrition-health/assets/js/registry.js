...
NH.registerLesson = function(data) {
  NH.lessons[data.id] = data;
  NH.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
...
NH.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < NH.searchIndex.length; i++) {
    var item = NH.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < NH.terms.length; t++) {
    var term = NH.terms[t];
    var ts = 0;
