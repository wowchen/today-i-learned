...
NH.render = function(html) {
  document.getElementById('app').innerHTML = html;
  NH.bindTerms();
};

NH.findTerm = function(id) {
  for (var i = 0; i < NH.terms.length; i++) {
    if (NH.terms[i].id === id) return NH.terms[i];
  }
  return null;
};

NH.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      NH.showTermCard(id, this);
    });
  }
};

NH.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = NH.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + NH.esc(term.name) + '</b> <span class="tc-en">' + NH.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + NH.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + NH.esc(term.analogy) + '</div>' : '') +
