...
(function () {
  ...
  function apiUrl() {
    // cfg.repo 形如 "owner/wah-progress"
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/wah.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  function pull() {
    ...
    return fetch(apiUrl(), { headers: headers() }).then(function (res) {
      ...
      return res.json().then(function (j) {
        lastSha = j.sha;
        WAH.progress().merge(JSON.parse(b64decodeUtf8(j.content)));
        return true;
      });
    });
  }
  ...
  function push(retry) {
    ...
    var body = {
      ...
      content: b64encodeUtf8(WAH.progress().exportJson()),
  ...
  WAH.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/wah.json').trim();
      saveCfg();
    },
