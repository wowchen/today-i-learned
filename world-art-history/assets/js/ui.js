...
WAH.render = function(html) {
  document.getElementById('app').innerHTML = html;
  WAH.bindTerms();
};

WAH.findTerm = function(id) {
  for (var i = 0; i < WAH.terms.length; i++) {
    if (WAH.terms[i].id === id) return WAH.terms[i];
  }
  return null;
};

WAH.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      WAH.showTermCard(id, this);
    });
  }
};

WAH.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = WAH.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + WAH.esc(term.name) + '</b> <span class="tc-en">' + WAH.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + WAH.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + WAH.esc(term.analogy) + '</div>' : '') +
