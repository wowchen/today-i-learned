...
WAH.views.lesson = function(id) {
  var l = WAH.lessons[id];
  if (!l) {
    WAH.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + WAH.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = WAH.path.indexOf(id);
  var stepNum = pathIdx >= 0 ? pathIdx + 1 : '';
  var mod = WAH.modules.find(function(m) { return m.id === l.module; });

  var html = '<article class="lesson">';
  html += '<div class="lesson-head">';
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? WAH.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + WAH.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = WAH.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="WAH.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="WAH.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < WAH.path.length - 1) {
    var next = WAH.path[pathIdx + 1];
...
WAH.markAndNext = function(id) {
  var P = WAH.progress();
  P.markRead(id);
  var idx = WAH.path.indexOf(id);
  if (idx < WAH.path.length - 1) {
    window.location.hash = '#/l/' + WAH.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + WAH.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + WAH.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
WAH.toggleRead = function(id) {
  var P = WAH.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  WAH.views.lesson(id);
  if (WAH.renderShell) WAH.renderShell();
};
