...
WAH.registerLesson = function(data) {
  WAH.lessons[data.id] = data;
  WAH.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
WAH.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < WAH.searchIndex.length; i++) {
    var item = WAH.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < WAH.terms.length; t++) {
    var term = WAH.terms[t];
    var ts = 0;
