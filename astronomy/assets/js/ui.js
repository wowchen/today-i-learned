...
AST.render = function(html) {
  document.getElementById('app').innerHTML = html;
  AST.bindTerms();
};

AST.findTerm = function(id) {
  for (var i = 0; i < AST.terms.length; i++) {
    if (AST.terms[i].id === id) return AST.terms[i];
  }
  return null;
};

AST.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      AST.showTermCard(id, this);
    });
  }
};

AST.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = AST.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + AST.esc(term.name) + '</b> <span class="tc-en">' + AST.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + AST.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + AST.esc(term.analogy) + '</div>' : '') +
