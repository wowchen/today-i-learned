...
AST.registerLesson = function(data) {
  AST.lessons[data.id] = data;
  AST.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
AST.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < AST.searchIndex.length; i++) {
    var item = AST.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < AST.terms.length; t++) {
    var term = AST.terms[t];
    var ts = 0;
