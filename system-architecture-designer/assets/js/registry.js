...
SAD.registerLesson = function(data) {
  SAD.lessons[data.id] = data;
  SAD.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
SAD.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < SAD.searchIndex.length; i++) {
    var item = SAD.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < SAD.terms.length; t++) {
    var term = SAD.terms[t];
    var ts = 0;
