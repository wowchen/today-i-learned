...
SAD.render = function(html) {
  document.getElementById('app').innerHTML = html;
  SAD.bindTerms();
};

SAD.findTerm = function(id) {
  for (var i = 0; i < SAD.terms.length; i++) {
    if (SAD.terms[i].id === id) return SAD.terms[i];
  }
  return null;
};

SAD.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      SAD.showTermCard(id, this);
    });
  }
};

SAD.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = SAD.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + SAD.esc(term.name) + '</b> <span class="tc-en">' + SAD.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + SAD.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + SAD.esc(term.analogy) + '</div>' : '') +
