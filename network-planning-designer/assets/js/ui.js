...
NPD.render = function(html) {
  document.getElementById('app').innerHTML = html;
  NPD.bindTerms();
};

NPD.findTerm = function(id) {
  for (var i = 0; i < NPD.terms.length; i++) {
    if (NPD.terms[i].id === id) return NPD.terms[i];
  }
  return null;
};

NPD.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      NPD.showTermCard(id, this);
    });
  }
};

NPD.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = NPD.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + NPD.esc(term.name) + '</b> <span class="tc-en">' + NPD.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + NPD.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + NPD.esc(term.analogy) + '</div>' : '') +
