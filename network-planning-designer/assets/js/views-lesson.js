...
NPD.views.lesson = function(id) {
  var l = NPD.lessons[id];
  if (!l) {
    NPD.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + NPD.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = NPD.path.indexOf(id);
  ...
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? NPD.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + NPD.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = NPD.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="NPD.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="NPD.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < NPD.path.length - 1) {
    var next = NPD.path[pathIdx + 1];
...
NPD.markAndNext = function(id) {
  var P = NPD.progress();
  P.markRead(id);
  var idx = NPD.path.indexOf(id);
  if (idx < NPD.path.length - 1) {
    window.location.hash = '#/l/' + NPD.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + NPD.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + NPD.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
NPD.toggleRead = function(id) {
  var P = NPD.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  NPD.views.lesson(id);
  if (NPD.renderShell) NPD.renderShell();
};
