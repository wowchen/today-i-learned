...
NPD.registerLesson = function(data) {
  NPD.lessons[data.id] = data;
  NPD.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
NPD.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < NPD.searchIndex.length; i++) {
    var item = NPD.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < NPD.terms.length; t++) {
    var term = NPD.terms[t];
    var ts = 0;
