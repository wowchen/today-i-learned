...
EMK.render = function(html) {
  document.getElementById('app').innerHTML = html;
  EMK.bindTerms();
};

EMK.findTerm = function(id) {
  for (var i = 0; i < EMK.terms.length; i++) {
    if (EMK.terms[i].id === id) return EMK.terms[i];
  }
  return null;
};

EMK.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      EMK.showTermCard(id, this);
    });
  }
};

EMK.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = EMK.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + EMK.esc(term.name) + '</b> <span class="tc-en">' + EMK.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + EMK.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + EMK.esc(term.analogy) + '</div>' : '') +
