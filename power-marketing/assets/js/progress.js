...
(function() {
  ...
  function save(data) {
    ...
    if (EMK.sync) EMK.sync.schedulePush();
    if (EMK.onProgressChange) EMK.onProgressChange();
  }

  EMK.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < EMK.path.length; i++) {
          if (!this.isRead(EMK.path[i])) return i;
        }
        return EMK.path.length;
      },
