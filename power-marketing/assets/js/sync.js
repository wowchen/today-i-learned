/* GitHub 进度同步(可选):fine-grained PAT + 独立 private repo + Contents API。
   ...
   注意:本站 progress 是工厂(EMK.progress() 每次返回新对象),故同步时带括号调用。 */
(function () {
  'use strict';
  window.EMK = window.EMK || {};

  var CFG_KEY = 'emk.sync.v1';
  var cfg = loadCfg();
  var pushTimer = null;
  var busy = false;
  var lastSha = null;
  ...
  function apiUrl() {
    ...
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/power-marketing.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  function pull() {
    ...
    return fetch(apiUrl(), { headers: headers() }).then(function (res) {
      ...
      return res.json().then(function (j) {
        lastSha = j.sha;
        EMK.progress().merge(JSON.parse(b64decodeUtf8(j.content)));
        return true;
      });
    });
  }
  ...
  function push(retry) {
    ...
    var body = {
      ...
      content: b64encodeUtf8(EMK.progress().exportJson()),
  ...
  EMK.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/power-marketing.json').trim();
      saveCfg();
    },
