...
EMK.registerLesson = function(data) {
  EMK.lessons[data.id] = data;
  EMK.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
EMK.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < EMK.searchIndex.length; i++) {
    var item = EMK.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < EMK.terms.length; t++) {
    var term = EMK.terms[t];
    var ts = 0;
