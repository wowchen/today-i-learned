...
EMK.views.lesson = function(id) {
  var l = EMK.lessons[id];
  if (!l) {
    EMK.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + EMK.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = EMK.path.indexOf(id);
  var stepNum = pathIdx >= 0 ? pathIdx + 1 : '';
  var mod = EMK.modules.find(function(m) { return m.id === l.module; });

  var html = '<article class="lesson">';
  html += '<div class="lesson-head">';
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? EMK.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + EMK.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = EMK.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="EMK.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="EMK.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < EMK.path.length - 1) {
    var next = EMK.path[pathIdx + 1];
...
EMK.markAndNext = function(id) {
  var P = EMK.progress();
  P.markRead(id);
  var idx = EMK.path.indexOf(id);
  if (idx < EMK.path.length - 1) {
    window.location.hash = '#/l/' + EMK.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + EMK.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + EMK.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
EMK.toggleRead = function(id) {
  var P = EMK.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  EMK.views.lesson(id);
  if (EMK.renderShell) EMK.renderShell();
};
