...
TSP.views.lesson = function(id) {
  var l = TSP.lessons[id];
  if (!l) {
    TSP.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + TSP.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = TSP.path.indexOf(id);
  var stepNum = pathIdx >= 0 ? pathIdx + 1 : '';
  var mod = TSP.modules.find(function(m) { return m.id === l.module; });

  var html = '<article class="lesson">';
  html += '<div class="lesson-head">';
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? TSP.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + TSP.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = TSP.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="TSP.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="TSP.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < TSP.path.length - 1) {
    var next = TSP.path[pathIdx + 1];
...
TSP.markAndNext = function(id) {
  var P = TSP.progress();
  P.markRead(id);
  var idx = TSP.path.indexOf(id);
  if (idx < TSP.path.length - 1) {
    window.location.hash = '#/l/' + TSP.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + TSP.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + TSP.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
TSP.toggleRead = function(id) {
  var P = TSP.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  TSP.views.lesson(id);
  if (TSP.renderShell) TSP.renderShell();
};
