...
TSP.render = function(html) {
  document.getElementById('app').innerHTML = html;
  TSP.bindTerms();
};

TSP.findTerm = function(id) {
  for (var i = 0; i < TSP.terms.length; i++) {
    if (TSP.terms[i].id === id) return TSP.terms[i];
  }
  return null;
};

TSP.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      TSP.showTermCard(id, this);
    });
  }
};

TSP.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = TSP.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + TSP.esc(term.name) + '</b> <span class="tc-en">' + TSP.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + TSP.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + TSP.esc(term.analogy) + '</div>' : '') +
