...
TSP.registerLesson = function(data) {
  TSP.lessons[data.id] = data;
  TSP.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
TSP.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < TSP.searchIndex.length; i++) {
    var item = TSP.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < TSP.terms.length; t++) {
    var term = TSP.terms[t];
    var ts = 0;
