...
(function() {
  ...
  function save(data) {
    ...
    if (TSP.sync) TSP.sync.schedulePush();
    if (TSP.onProgressChange) TSP.onProgressChange();
  }

  TSP.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < TSP.path.length; i++) {
          if (!this.isRead(TSP.path[i])) return i;
        }
        return TSP.path.length;
      },
