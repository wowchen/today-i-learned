...
(function() {
  ...
  function save(data) {
    ...
    if (PFIN.sync) PFIN.sync.schedulePush();
    if (PFIN.onProgressChange) PFIN.onProgressChange();
  }

  PFIN.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < PFIN.path.length; i++) {
          if (!this.isRead(PFIN.path[i])) return i;
        }
        return PFIN.path.length;
      },
