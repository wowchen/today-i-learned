...
PFIN.registerLesson = function(data) {
  PFIN.lessons[data.id] = data;
  PFIN.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
PFIN.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < PFIN.searchIndex.length; i++) {
    var item = PFIN.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < PFIN.terms.length; t++) {
    var term = PFIN.terms[t];
    var ts = 0;
