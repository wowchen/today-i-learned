/* GitHub 进度同步(可选):fine-grained PAT + 独立 private repo + Contents API。
   ...
   旧版站点说明:progress 为工厂(PFIN.progress()),故 merge/exportJson 需带 () 调用。 */
(function () {
  'use strict';
  window.PFIN = window.PFIN || {};
  var NS = window.PFIN;

  var CFG_KEY = 'pfin.sync.v1';
  var cfg = loadCfg();
  var pushTimer = null;
  var busy = false;
  var lastSha = null;
  ...
  function apiUrl() {
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/pfin.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  function pull() {
    ...
    return fetch(apiUrl(), { headers: headers() }).then(function (res) {
      ...
      return res.json().then(function (j) {
        lastSha = j.sha;
        NS.progress().merge(JSON.parse(b64decodeUtf8(j.content)));
        return true;
      });
    });
  }
  ...
  function push(retry) {
    ...
    var body = {
      ...
      content: b64encodeUtf8(NS.progress().exportJson()),
  ...
  NS.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/pfin.json').trim();
      saveCfg();
    },
