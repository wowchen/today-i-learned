...
AIP.render = function(html) {
  document.getElementById('app').innerHTML = html;
  AIP.bindTerms();
};

AIP.findTerm = function(id) {
  for (var i = 0; i < AIP.terms.length; i++) {
    if (AIP.terms[i].id === id) return AIP.terms[i];
  }
  return null;
};

AIP.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      AIP.showTermCard(id, this);
    });
  }
};

AIP.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = AIP.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + AIP.esc(term.name) + '</b> <span class="tc-en">' + AIP.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + AIP.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + AIP.esc(term.analogy) + '</div>' : '') +
