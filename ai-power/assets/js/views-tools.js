...
AIP.views.settings = function() {
  ...
  /* ── GitHub 进度同步(可选) ── */
  var syncCfg = AIP.sync.config();
  html += '<h3>GitHub 进度同步（可选）</h3>';
  html += '<p class="calc-intro">用一个<b>自己的 private 仓库</b>存放进度（如 you/aip-progress），fine-grained PAT 仅授权该仓库的 Contents 读写。token 只存本机浏览器，不会进入站点代码仓库。不配置则进度仅存本机，不影响学习。</p>';
  html += '<div class="setting-row"><label>仓库</label><input id="syRepo" placeholder="owner/repo" value="' + AIP.esc(syncCfg.repo || '') + '"></div>';
  html += '<div class="setting-row"><label>分支</label><input id="syBranch" placeholder="main" value="' + AIP.esc(syncCfg.branch || 'main') + '"></div>';
  html += '<div class="setting-row"><label>Token</label><input id="syToken" type="password" placeholder="github_pat_…" value="' + AIP.esc(syncCfg.token || '') + '"></div>';
  ...
  html += '<p class="calc-intro" id="syMsg">' + AIP.esc((AIP.sync && AIP.sync.statusText) || '仅本机') + '</p>';
  ...
  if (sySaveEl) {
    sySaveEl.addEventListener('click', function () {
      ...
      if (!AIP.sync.ready()) { syncMsg('仓库和 token 都要填。', 'bad'); return; }
      syncMsg('同步中…');
      AIP.sync.pullNow().then(function () { return AIP.sync.pushNow(); })
        .then(function (ok) { syncMsg(ok ? '已同步 ✓ 两台设备现在看到同一份进度。' : '同步失败：' + AIP.sync.statusText, ok ? 'ok' : 'bad'); });
    });
  }
  ...
  if (syPullEl) {
    syPullEl.addEventListener('click', function () {
      syncMsg('拉取中…');
      AIP.sync.pullNow().then(function (ok) { syncMsg(ok ? '已拉取并合并远端进度 ✓' : AIP.sync.statusText, ok ? 'ok' : ''); });
    });
  }
