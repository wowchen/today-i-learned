...
AIP.registerLesson = function(data) {
  AIP.lessons[data.id] = data;
  AIP.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
AIP.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < AIP.searchIndex.length; i++) {
    var item = AIP.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < AIP.terms.length; t++) {
    var term = AIP.terms[t];
    var ts = 0;
