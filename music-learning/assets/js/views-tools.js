...
(function () {
  ...
  MUS.views.terms = function () {
    ...
    var groups = R().modules().filter(function (m) { return byModule[m.id]; })
      .map(function (m) {
        return '<div class="glossary-group">' +
          '<div class="chapter"><span>模块 ' + m.no + '</span><h2>' + MUS.esc(m.title) + '</h2></div>' +
    ...
    MUS.render(
      ...
      '<p class="sub">十五五的"政策词"都在这里,随内容更新。点术语名弹出卡片,可收藏进自己的术语本。</p>' +
      '<div class="searchbox"><input id="tq" type="search" placeholder="搜术语,比如:约束性指标 / 新质生产力 / 碳达峰"></div>' +
    ...
    input.addEventListener('input', function () {
      ...
      t = setTimeout(function () {
        ...
        box.innerHTML = hits.length
          ...
          : '<p class="empty">没有找到「' + MUS.esc(q) + '」。</p>';
      }, 180);
    });
  };
  ...
  function termRow(t) {
    var src = t.lesson && R().lesson(t.lesson)
      ? '<a class="src" href="#/l/' + t.lesson + '">出自:' + MUS.esc(R().lesson(t.lesson).title) + '</a>' : '';
    return '<li><span class="term" data-termpop data-term="' + MUS.esc(t.id) + '">' + MUS.esc(t.name) + '</span>' +
      '<span class="def">' + MUS.esc(t.def) + '</span>' + src + '</li>';
  }

  /* ── 我的术语本 ── */
  MUS.views.book = function () {
    var favs = P().listTerms();
    var list = favs.length ? '<ul class="terms">' + favs.map(function (f) {
      var t = R().term(f._id);
      if (!t) return '';
      var src = f.lesson && R().lesson(f.lesson)
        ? '<a class="src" href="#/l/' + f.lesson + '">出自:' + MUS.esc(R().lesson(f.lesson).title) + '</a>' : '';
      return '<li><span class="term" data-termpop data-term="' + MUS.esc(t.id) + '">' + MUS.esc(t.name) + '</span>' +
        '<span class="def">' + MUS.esc(t.def) + '</span>' + src +
        '<button class="del" data-del="' + MUS.esc(t.id) + '">删除</button></li>';
    }).join('') + '</ul>'
    ...
    MUS.render(
      ...
      (favs.length ? '<div style="margin-top:26px"><a class="btn ghost" href="#/review">用它们翻牌复习 ' + MUS.icon('arrowR', 12) + '</a></div>' : '')
    );
  ...
  /* ── 翻牌复习:随机抽 5 张,纯回顾不判分 ── */
  MUS.views.review = function () {
    var pool = P().listTerms().map(function (f) { return R().term(f._id); })
      .filter(function (t) { return !!t; });
    if (!pool.length) {
      MUS.render('<p class="empty">术语本还是空的,先去课程里收藏几个术语吧。<br><br><a class="btn solid" href="#/">去学习</a></p>');
      return;
    }
    ...
    MUS.render(
      ...
      '<button class="btn solid" id="nextBtn">下一张 ' + MUS.icon('arrowR', 12) + '</button>' +
    ...
    function draw() {
      ...
      if (!flipped) {
        face.innerHTML = '<p class="big">' + MUS.esc(c.name) + '</p>' +
          (c.en ? '<p class="small">' + MUS.esc(c.en) + '</p>' : '') +
          '<p class="small">这个术语什么意思?想好再翻面</p>';
      } else {
        face.innerHTML = '<p style="font-size:16.5px;max-width:34em">' + MUS.esc(c.def) + '</p>' +
          (c.analogy ? '<p class="small">类比:' + MUS.esc(c.analogy) + '</p>' : '') +
          (lessonTitle ? '<p class="small">出自:' + MUS.esc(lessonTitle) + '</p>' : '');
      }
    ...
    document.getElementById('reBtn').addEventListener('click', MUS.views.review);
    draw();
  };

  /* ── 搜索 ── */
  MUS.views.search = function () {
    MUS.render(
      '<div class="chapter" style="margin-top:30px"><span>检索</span><h2>全文搜索</h2></div>' +
      '<p class="sub">搜课程标题、关键词、正文,也搜术语。索引由内容数据自动生成。</p>' +
      '<div class="searchbox"><input id="sq" type="search" placeholder="比如:养老 / 人工智能 / 碳达峰 / 统一大市场"></div>' +
      '<div id="sr"></div>'
    );
        ...
        if (hits.terms.length) {
          html += '<div class="chapter"><span>术语</span><h2>' + hits.terms.length + ' 条</h2></div>' +
            '<ul class="terms">' + hits.terms.map(function (tm) {
              return '<li><span class="term" data-termpop data-term="' + MUS.esc(tm.id) + '">' +
                MUS.esc(tm.name) + '</span><span class="def">' + MUS.esc(tm.def) + '</span></li>';
            }).join('') + '</ul>';
        }
        if (hits.lessons.length) {
          html += '<div class="chapter"><span>课程</span><h2>' + hits.lessons.length + ' 课</h2></div>' +
            hits.lessons.map(function (h) {
              ...
              return '<div class="result"><p class="crumb">' + MUS.esc(mod.title) + '</p>' +
                '<a href="#/l/' + h.lesson.id + '">' + MUS.esc(h.lesson.title) + '</a>' +
                '<p>' + MUS.esc(h.snippet) + '</p></div>';
            }).join('');
        }
        box.innerHTML = html || '<p class="empty">没有找到「' + MUS.esc(q) + '」——可能这部分内容还没编写。</p>';
      }, 200);
    });
  };

  /* ── 设置 ── */
  MUS.views.settings = function () {
    var cfg = MUS.sync.config();
    MUS.render(
      ...
      '<button data-v="dark">黑胶夜场(深)</button><button data-v="light">纸上乐谱(浅)</button></span></div>' +
      ...
      '<p class="d">用一个<b>自己的 private 仓库</b>存进度(如 you/music-progress),fine-grained PAT 只授权该仓库的 Contents 读写、建议设 90 天过期。token 只存在本机浏览器,不会进入站点代码仓库。不配置则进度仅存本机,不影响学习。</p>' +
      '<div class="set-row"><label>仓库</label><input id="syRepo" placeholder="owner/ffp-progress" value="' + MUS.esc(cfg.repo || '') + '"></div>' +
      '<div class="set-row"><label>分支</label><input id="syBranch" placeholder="main" value="' + MUS.esc(cfg.branch || 'main') + '"></div>' +
      '<div class="set-row"><label>Token</label><input id="syToken" type="password" placeholder="github_pat_…" value="' + MUS.esc(cfg.token || '') + '"></div>' +
      ...
      '<p class="sync-msg" id="syMsg">' + MUS.esc(MUS.sync.statusText) + '</p></div>' +

      '<div class="card set-group"><h3>手动备份</h3>' +
      '<p class="d">自动同步之外的备用通道:导出/导入 mus-progress.json。导入采用智能合并,不会覆盖丢数据。</p>' +
      '<div class="set-row"><button class="btn" id="expBtn">导出 mus-progress.json</button>' +
    ...
    document.getElementById('sySave').addEventListener('click', function () {
      ...
      if (!MUS.sync.ready()) { msg('仓库和 token 都要填。', 'bad'); return; }
      msg('同步中…');
      MUS.sync.pullNow().then(function () { return MUS.sync.pushNow(); })
        .then(function (ok) { msg(ok ? '已同步 ✓ 两台设备现在看到同一份进度。' : '同步失败:' + MUS.sync.statusText, ok ? 'ok' : 'bad'); });
    });
    document.getElementById('syPull').addEventListener('click', function () {
      msg('拉取中…');
      MUS.sync.pullNow().then(function (ok) { msg(ok ? '已拉取并合并远端进度 ✓' : MUS.sync.statusText, ok ? 'ok' : ''); });
    });
    ...
    document.getElementById('impFile').addEventListener('change', function (e) {
      ...
      fr.onload = function () {
        alert(P().importJson(fr.result) ? '导入成功,已与本机数据合并。' : '导入失败:不是有效的 mus-progress.json。');
      };
      fr.readAsText(f);
    });
