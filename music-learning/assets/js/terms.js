...
(function () {
  ...
  function lookup(g) {
    ...
    if (id) {
      var t = MUS.registry.term(id);
      if (t) return t;
    }
    return MUS.registry.termByName(g.textContent.trim());
  }
  ...
  function show(g) {
    ...
    if (!t) {
      ...
      p.innerHTML = '<div class="t">' + MUS.esc(g.textContent.trim()) + '</div>' +
        '<div class="a">这个术语的卡片还没编写。</div>';
    } else {
      var faved = MUS.progress.hasTerm(t.id);
      var lessonLink = '';
      if (t.lesson && MUS.registry.lesson(t.lesson)) {
        lessonLink = '<a href="#/l/' + t.lesson + '">去那一课 ' + MUS.icon('arrowR', 12) + '</a>';
      }
      p.innerHTML =
        '<div class="t">' + MUS.esc(t.name) +
        (t.en ? '<small>' + MUS.esc(t.en) + '</small>' : '') + '</div>' +
        '<div class="d">' + MUS.esc(t.def) + '</div>' +
        (t.analogy ? '<div class="a">类比:' + MUS.esc(t.analogy) + '</div>' : '') +
        '<div class="ops">' +
        (faved
          ? '<button type="button" class="faved">已在术语本 ✓</button>'
          : '<button type="button" class="fav">' + MUS.icon('star', 12) + ' 收藏</button>') +
        lessonLink + '</div>';
      var favBtn = p.querySelector('.fav');
      if (favBtn) favBtn.onclick = function (ev) {
        ev.stopPropagation();
        MUS.progress.addTerm(t.id, MUS.currentLessonId);
