...
(function () {
  ...
  function save() {
    ...
    if (MUS.sync) MUS.sync.schedulePush();
    if (MUS.onProgressChange) MUS.onProgressChange();
  }
  ...
  MUS.progress = {
    ...
    readCountOf: function (moduleId) {
      var n = 0;
      MUS.registry.lessonsOf(moduleId).forEach(function (l) {
        if (MUS.progress.isRead(l.id)) n++;
      });
      return n;
    },
