...
(function () {
  ...
  var CFG_KEY = 'mus.sync.v1';
  var cfg = loadCfg();
  /* 迁移:早期版本默认路径为裸 progress.json(落在仓库根),
     现统一为 progress/mus.json,避免多站同步到同一仓库时互相覆盖。 */
  function migrateLegacyPath() {
    if (cfg.path === 'progress.json') { cfg.path = 'progress/mus.json'; saveCfg(); }
  }
  migrateLegacyPath();
  var pushTimer = null;
  var busy = false;
  ...
  function pull() {
    ...
    return fetch(apiUrl(), { headers: headers() }).then(function (res) {
      ...
      return res.json().then(function (j) {
        lastSha = j.sha;
        MUS.progress.merge(JSON.parse(b64decodeUtf8(j.content)));
        return true;
      });
    });
  }
  ...
  function push(retry) {
    ...
    var body = {
      ...
      content: b64encodeUtf8(MUS.progress.exportJson()),
