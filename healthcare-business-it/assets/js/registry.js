...
HIT.registerLesson = function(data) {
  HIT.lessons[data.id] = data;
  HIT.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
HIT.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < HIT.searchIndex.length; i++) {
    var item = HIT.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < HIT.terms.length; t++) {
    var term = HIT.terms[t];
    var ts = 0;
