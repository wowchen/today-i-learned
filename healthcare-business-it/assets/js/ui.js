...
HIT.findTerm = function(id) {
  for (var i = 0; i < HIT.terms.length; i++) {
    if (HIT.terms[i].id === id) return HIT.terms[i];
  }
  return null;
};

HIT.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      HIT.showTermCard(id, this);
    });
  }
};

HIT.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = HIT.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + HIT.esc(term.name) + '</b> <span class="tc-en">' + HIT.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + HIT.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + HIT.esc(term.analogy) + '</div>' : '') +
