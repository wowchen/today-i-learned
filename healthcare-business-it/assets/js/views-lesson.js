...
HIT.views.lesson = function(id) {
  var l = HIT.lessons[id];
  if (!l) {
    HIT.render('<div class="empty-state"><h2 class="page">课时待编写</h2>' +
      '<p>ID: ' + HIT.esc(id) + '</p><a class="btn" href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = HIT.path.indexOf(id);
  var mod = HIT.modules.find(function(m) { return m.id === l.module; });
  ...
  h += '<div class="pager">';
  if (pathIdx > 0) {
    var prev = HIT.path[pathIdx - 1];
    h += '<a href="#/l/' + prev + '">上一课<b>' + esc(HIT.lessons[prev] ? HIT.lessons[prev].title : prev) + '</b></a>';
  }
  if (!P.isRead(id)) {
  ...
  if (pathIdx >= 0 && pathIdx < HIT.path.length - 1) {
    ...
    h += '<a class="next" href="#/l/' + next + '">下一课<b>' + esc(HIT.lessons[next] ? HIT.lessons[next].title : next) + '</b></a>';
  }
  h += '</div>';
...
HIT.markAndNext = function(id) {
  var P = HIT.progress();
  P.markRead(id);
  var idx = HIT.path.indexOf(id);
  if (idx >= 0 && idx < HIT.path.length - 1) {
    window.location.hash = '#/l/' + HIT.path[idx + 1];
  } else {
    HIT.go('#/');
  }
