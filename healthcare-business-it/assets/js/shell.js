...
(function() {
  var TOOLS = [
    { v: 'timeline',   hash: '#/timeline',  label: '时间轴' },
    { v: 'figures',    hash: '#/figures',   label: '角色图鉴' },
    { v: 'cheatsheet', hash: '#/cheatsheet',label: '缩略语速查' },
    { v: 'compare',    hash: '#/compare',   label: '流程对照' },
  ...
  function lessonsOf(moduleId) {
    ...
    for (var i = 0; i < HIT.path.length; i++) {
      if (HIT.path[i].indexOf(moduleId + '/') === 0) out.push(HIT.path[i]);
    }
    return out;
  }
  ...
  HIT.renderShell = function() {
    ...
    var h = '';
    h += '<a class="brand" href="#/"><span class="mk">HIT</span>' +
         '<span><b>医疗业务与信息化</b><small>HEALTHCARE · BLUEPRINT</small></span></a>';
    ...
    if (tb) tb.addEventListener('click', HIT.toggleTheme);
  };

  HIT.closeDrawer = function() {
    var shell = document.getElementById('app-shell');
    if (shell) shell.classList.remove('drawer');
  };

  HIT.buildShell = function() {
