...
FIT.render = function(html) {
  document.getElementById('app').innerHTML = html;
  FIT.bindTerms();
};

FIT.findTerm = function(id) {
  for (var i = 0; i < FIT.terms.length; i++) {
    if (FIT.terms[i].id === id) return FIT.terms[i];
  }
  return null;
};

FIT.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      FIT.showTermCard(id, this);
    });
  }
};

FIT.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = FIT.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + FIT.esc(term.name) + '</b> <span class="tc-en">' + FIT.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + FIT.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + FIT.esc(term.analogy) + '</div>' : '') +
