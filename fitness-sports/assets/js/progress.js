...
(function() {
  ...
  function save(data) {
    ...
    if (FIT.sync) FIT.sync.schedulePush();
    if (FIT.onProgressChange) FIT.onProgressChange();
  }

  FIT.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < FIT.path.length; i++) {
          if (!this.isRead(FIT.path[i])) return i;
        }
        return FIT.path.length;
      },
