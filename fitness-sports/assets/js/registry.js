...
FIT.registerLesson = function(data) {
  FIT.lessons[data.id] = data;
  FIT.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
FIT.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < FIT.searchIndex.length; i++) {
    var item = FIT.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < FIT.terms.length; t++) {
    var term = FIT.terms[t];
    var ts = 0;
