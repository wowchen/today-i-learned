...
TCM.registerLesson = function(data) {
  TCM.lessons[data.id] = data;
  TCM.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
TCM.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < TCM.searchIndex.length; i++) {
    var item = TCM.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < TCM.terms.length; t++) {
    var term = TCM.terms[t];
    var ts = 0;
