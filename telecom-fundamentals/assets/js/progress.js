...
(function() {
  ...
  function save(data) {
    ...
    if (TCM.sync) TCM.sync.schedulePush();
    if (TCM.onProgressChange) TCM.onProgressChange();
  }

  TCM.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < TCM.path.length; i++) {
          if (!this.isRead(TCM.path[i])) return i;
        }
        return TCM.path.length;
      },
