...
TCM.render = function(html) {
  document.getElementById('app').innerHTML = html;
  TCM.bindTerms();
};

TCM.findTerm = function(id) {
  for (var i = 0; i < TCM.terms.length; i++) {
    if (TCM.terms[i].id === id) return TCM.terms[i];
  }
  return null;
};

TCM.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      TCM.showTermCard(id, this);
    });
  }
};

TCM.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = TCM.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + TCM.esc(term.name) + '</b> <span class="tc-en">' + TCM.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + TCM.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + TCM.esc(term.analogy) + '</div>' : '') +
