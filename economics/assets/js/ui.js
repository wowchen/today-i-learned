...
ECON.render = function(html) {
  document.getElementById('app').innerHTML = html;
  ECON.bindTerms();
};

ECON.findTerm = function(id) {
  for (var i = 0; i < ECON.terms.length; i++) {
    if (ECON.terms[i].id === id) return ECON.terms[i];
  }
  return null;
};

ECON.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      ECON.showTermCard(id, this);
    });
  }
};

ECON.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = ECON.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + ECON.esc(term.name) + '</b> <span class="tc-en">' + ECON.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + ECON.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + ECON.esc(term.analogy) + '</div>' : '') +
