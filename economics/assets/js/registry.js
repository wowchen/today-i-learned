...
ECON.registerLesson = function(data) {
  ECON.lessons[data.id] = data;
  ECON.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
ECON.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < ECON.searchIndex.length; i++) {
    var item = ECON.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < ECON.terms.length; t++) {
    var term = ECON.terms[t];
    var ts = 0;
