...
ECON.views.lesson = function(id) {
  var l = ECON.lessons[id];
  if (!l) {
    ECON.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + ECON.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = ECON.path.indexOf(id);
  var stepNum = pathIdx >= 0 ? pathIdx + 1 : '';
  var mod = ECON.modules.find(function(m) { return m.id === l.module; });

  var html = '<article class="lesson">';
  html += '<div class="lesson-head">';
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? ECON.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + ECON.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = ECON.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="ECON.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="ECON.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < ECON.path.length - 1) {
    var next = ECON.path[pathIdx + 1];
...
ECON.markAndNext = function(id) {
  var P = ECON.progress();
  P.markRead(id);
  var idx = ECON.path.indexOf(id);
  if (idx < ECON.path.length - 1) {
    window.location.hash = '#/l/' + ECON.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + ECON.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + ECON.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
