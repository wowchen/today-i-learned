...
GTR.registerLesson = function(data) {
  GTR.lessons[data.id] = data;
  GTR.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
GTR.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < GTR.searchIndex.length; i++) {
    var item = GTR.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < GTR.terms.length; t++) {
    var term = GTR.terms[t];
    var ts = 0;
