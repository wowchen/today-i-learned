...
(function() {
  ...
  function save(data) {
    ...
    if (GTR.sync) GTR.sync.schedulePush();
    if (GTR.onProgressChange) GTR.onProgressChange();
  }

  GTR.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < GTR.path.length; i++) {
          if (!this.isRead(GTR.path[i])) return i;
        }
        return GTR.path.length;
      },
