...
GTR.render = function(html) {
  document.getElementById('app').innerHTML = html;
  GTR.bindTerms();
};

GTR.findTerm = function(id) {
  for (var i = 0; i < GTR.terms.length; i++) {
    if (GTR.terms[i].id === id) return GTR.terms[i];
  }
  return null;
};

GTR.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      GTR.showTermCard(id, this);
    });
  }
};

GTR.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = GTR.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + GTR.esc(term.name) + '</b> <span class="tc-en">' + GTR.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + GTR.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + GTR.esc(term.analogy) + '</div>' : '') +
