...
GTR.views.lesson = function(id) {
  var l = GTR.lessons[id];
  if (!l) {
    GTR.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + GTR.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = GTR.path.indexOf(id);
  var stepNum = pathIdx >= 0 ? pathIdx + 1 : '';
  var mod = GTR.modules.find(function(m) { return m.id === l.module; });

  var html = '<article class="lesson">';
  html += '<div class="lesson-head">';
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? GTR.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + GTR.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = GTR.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="GTR.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="GTR.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < GTR.path.length - 1) {
    var next = GTR.path[pathIdx + 1];
...
GTR.markAndNext = function(id) {
  var P = GTR.progress();
  P.markRead(id);
  var idx = GTR.path.indexOf(id);
  if (idx < GTR.path.length - 1) {
    window.location.hash = '#/l/' + GTR.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + GTR.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + GTR.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
GTR.toggleRead = function(id) {
  var P = GTR.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  GTR.views.lesson(id);
  if (GTR.renderShell) GTR.renderShell();
};
