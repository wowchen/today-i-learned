...
PDN.render = function(html) {
  document.getElementById('app').innerHTML = html;
  PDN.bindTerms();
};

PDN.findTerm = function(id) {
  for (var i = 0; i < PDN.terms.length; i++) {
    if (PDN.terms[i].id === id) return PDN.terms[i];
  }
  return null;
};

PDN.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      PDN.showTermCard(id, this);
    });
  }
};

PDN.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = PDN.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + PDN.esc(term.name) + '</b> <span class="tc-en">' + PDN.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + PDN.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + PDN.esc(term.analogy) + '</div>' : '') +
