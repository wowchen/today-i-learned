...
PDN.registerLesson = function(data) {
  PDN.lessons[data.id] = data;
  PDN.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
PDN.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < PDN.searchIndex.length; i++) {
    var item = PDN.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < PDN.terms.length; t++) {
    var term = PDN.terms[t];
    var ts = 0;
