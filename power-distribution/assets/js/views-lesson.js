...
PDN.views.lesson = function(id) {
  var l = PDN.lessons[id];
  if (!l) {
    PDN.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + PDN.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = PDN.path.indexOf(id);
  var stepNum = pathIdx >= 0 ? pathIdx + 1 : '';
  var mod = PDN.modules.find(function(m) { return m.id === l.module; });

  var html = '<article class="lesson">';
  html += '<div class="lesson-head">';
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? PDN.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + PDN.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = PDN.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="PDN.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="PDN.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < PDN.path.length - 1) {
    var next = PDN.path[pathIdx + 1];
...
PDN.markAndNext = function(id) {
  var P = PDN.progress();
  P.markRead(id);
  var idx = PDN.path.indexOf(id);
  if (idx < PDN.path.length - 1) {
    window.location.hash = '#/l/' + PDN.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + PDN.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + PDN.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
PDN.toggleRead = function(id) {
  var P = PDN.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  PDN.views.lesson(id);
  if (PDN.renderShell) PDN.renderShell();
};
