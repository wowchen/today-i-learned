...
(function() {
  ...
  function save(data) {
    ...
    if (CCN.sync) CCN.sync.schedulePush();
    if (CCN.onProgressChange) CCN.onProgressChange();
  }

  CCN.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < CCN.path.length; i++) {
          if (!this.isRead(CCN.path[i])) return i;
        }
        return CCN.path.length;
      },
