...
CCN.render = function(html) {
  document.getElementById('app').innerHTML = html;
  CCN.bindTerms();
};

CCN.findTerm = function(id) {
  for (var i = 0; i < CCN.terms.length; i++) {
    if (CCN.terms[i].id === id) return CCN.terms[i];
  }
  return null;
};

CCN.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      CCN.showTermCard(id, this);
    });
  }
};

CCN.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = CCN.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + CCN.esc(term.name) + '</b> <span class="tc-en">' + CCN.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + CCN.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + CCN.esc(term.analogy) + '</div>' : '') +
