...
CCN.views.lesson = function(id) {
  var l = CCN.lessons[id];
  if (!l) {
    CCN.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + CCN.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = CCN.path.indexOf(id);
  var stepNum = pathIdx >= 0 ? pathIdx + 1 : '';
  var mod = CCN.modules.find(function(m) { return m.id === l.module; });

  var html = '<article class="lesson">';
  html += '<div class="lesson-head">';
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? CCN.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + CCN.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = CCN.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="CCN.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="CCN.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < CCN.path.length - 1) {
    var next = CCN.path[pathIdx + 1];
...
CCN.markAndNext = function(id) {
  var P = CCN.progress();
  P.markRead(id);
  var idx = CCN.path.indexOf(id);
  if (idx < CCN.path.length - 1) {
    window.location.hash = '#/l/' + CCN.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + CCN.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + CCN.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
CCN.toggleRead = function(id) {
  var P = CCN.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  CCN.views.lesson(id);
  if (CCN.renderShell) CCN.renderShell();
};
