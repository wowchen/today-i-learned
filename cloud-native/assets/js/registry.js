...
CCN.registerLesson = function(data) {
  CCN.lessons[data.id] = data;
  CCN.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
CCN.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < CCN.searchIndex.length; i++) {
    var item = CCN.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < CCN.terms.length; t++) {
    var term = CCN.terms[t];
    var ts = 0;
