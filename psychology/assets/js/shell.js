...
(function() {
  var TOOLS = [
    { v: 'timeline',   hash: '#/timeline',  label: '心理学大事记' },
    { v: 'figures',    hash: '#/figures',   label: '人物图鉴' },
    { v: 'cheatsheet', hash: '#/cheatsheet',label: '效应速查' },
    { v: 'compare',    hash: '#/compare',   label: '流派对照' },
  ...
  function lessonsOf(moduleId) {
    var out = [];
    for (var i = 0; i < PSY.path.length; i++) {
      if (PSY.path[i].indexOf(moduleId + '/') === 0) out.push(PSY.path[i]);
    }
    return out;
  }
  ...
  PSY.renderShell = function() {
    ...
    var h = '';
    h += '<a class="brand" href="#/"><span class="mk">PSY</span>' +
         '<span><b>心理学通识</b><small>PSYCHOLOGY · 通识</small></span></a>';
    ...
    if (tb) tb.addEventListener('click', PSY.toggleTheme);
  };

  PSY.closeDrawer = function() {
    var shell = document.getElementById('app-shell');
    if (shell) shell.classList.remove('drawer');
  };

  PSY.buildShell = function() {
