...
PSY.registerLesson = function(data) {
  PSY.lessons[data.id] = data;
  PSY.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
PSY.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < PSY.searchIndex.length; i++) {
    var item = PSY.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < PSY.terms.length; t++) {
    var term = PSY.terms[t];
    var ts = 0;
