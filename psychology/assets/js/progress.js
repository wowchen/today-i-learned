...
(function() {
  ...
  function save(data) {
    ...
    if (PSY.sync) PSY.sync.schedulePush();
    if (PSY.onProgressChange) PSY.onProgressChange();
  }

  PSY.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < PSY.path.length; i++) {
          if (!this.isRead(PSY.path[i])) return i;
        }
        return PSY.path.length;
      },
