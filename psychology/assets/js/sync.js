/* GitHub 进度同步(可选):fine-grained PAT + 独立 private repo + Contents API。
   ...
   旧版站点说明:progress 为工厂(PSY.progress()),故 merge/exportJson 需带 () 调用。 */
(function () {
  'use strict';
  window.PSY = window.PSY || {};
  var NS = window.PSY;

  var CFG_KEY = 'psy.sync.v1';
  var cfg = loadCfg();
  var pushTimer = null;
  var busy = false;
  var lastSha = null;
  ...
  function apiUrl() {
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/psy.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  NS.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/psy.json').trim();
      saveCfg();
    },
