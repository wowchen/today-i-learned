...
PSY.findTerm = function(id) {
  for (var i = 0; i < PSY.terms.length; i++) {
    if (PSY.terms[i].id === id) return PSY.terms[i];
  }
  return null;
};

PSY.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      PSY.showTermCard(id, this);
    });
  }
};

PSY.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = PSY.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + PSY.esc(term.name) + '</b> <span class="tc-en">' + PSY.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + PSY.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + PSY.esc(term.analogy) + '</div>' : '') +
