...
AIX.findTerm = function(id) {
  for (var i = 0; i < AIX.terms.length; i++) {
    if (AIX.terms[i].id === id) return AIX.terms[i];
  }
  return null;
};

AIX.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      AIX.showTermCard(id, this);
    });
  }
};

AIX.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = AIX.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + AIX.esc(term.name) + '</b> <span class="tc-en">' + AIX.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + AIX.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + AIX.esc(term.analogy) + '</div>' : '') +
