...
NPS.render = function(html) {
  document.getElementById('app').innerHTML = html;
  NPS.bindTerms();
};

NPS.findTerm = function(id) {
  for (var i = 0; i < NPS.terms.length; i++) {
    if (NPS.terms[i].id === id) return NPS.terms[i];
  }
  return null;
};

NPS.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      NPS.showTermCard(id, this);
    });
  }
};

NPS.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = NPS.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + NPS.esc(term.name) + '</b> <span class="tc-en">' + NPS.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + NPS.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + NPS.esc(term.analogy) + '</div>' : '') +
