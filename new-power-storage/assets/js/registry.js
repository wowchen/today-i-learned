...
NPS.registerLesson = function(data) {
  NPS.lessons[data.id] = data;
  NPS.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
NPS.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < NPS.searchIndex.length; i++) {
    var item = NPS.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < NPS.terms.length; t++) {
    var term = NPS.terms[t];
    var ts = 0;
