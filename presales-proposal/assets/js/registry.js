...
PRS.registerLesson = function(data) {
  PRS.lessons[data.id] = data;
  PRS.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
PRS.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < PRS.searchIndex.length; i++) {
    var item = PRS.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < PRS.terms.length; t++) {
    var term = PRS.terms[t];
    var ts = 0;
