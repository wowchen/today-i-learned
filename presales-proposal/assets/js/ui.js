...
PRS.render = function(html) {
  document.getElementById('app').innerHTML = html;
  PRS.bindTerms();
};

PRS.findTerm = function(id) {
  for (var i = 0; i < PRS.terms.length; i++) {
    if (PRS.terms[i].id === id) return PRS.terms[i];
  }
  return null;
};

PRS.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      PRS.showTermCard(id, this);
    });
  }
};

PRS.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = PRS.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + PRS.esc(term.name) + '</b> <span class="tc-en">' + PRS.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + PRS.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + PRS.esc(term.analogy) + '</div>' : '') +
