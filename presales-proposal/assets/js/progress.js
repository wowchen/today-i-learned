...
(function() {
  ...
  function save(data) {
    ...
    if (PRS.sync) PRS.sync.schedulePush();
    if (PRS.onProgressChange) PRS.onProgressChange();
  }

  PRS.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < PRS.path.length; i++) {
          if (!this.isRead(PRS.path[i])) return i;
        }
        return PRS.path.length;
      },
