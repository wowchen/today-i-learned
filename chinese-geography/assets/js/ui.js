...
CG.render = function(html) {
  document.getElementById('app').innerHTML = html;
  CG.bindTerms();
};

CG.findTerm = function(id) {
  for (var i = 0; i < CG.terms.length; i++) {
    if (CG.terms[i].id === id) return CG.terms[i];
  }
  return null;
};

CG.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      CG.showTermCard(id, this);
    });
  }
};

CG.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = CG.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + CG.esc(term.name) + '</b> <span class="tc-en">' + CG.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + CG.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + CG.esc(term.analogy) + '</div>' : '') +
