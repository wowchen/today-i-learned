...
CG.views.lesson = function(id) {
  var l = CG.lessons[id];
  if (!l) {
    CG.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + CG.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
  ...
  var pathIdx = CG.path.indexOf(id);
  var stepNum = pathIdx >= 0 ? pathIdx + 1 : '';
  var mod = CG.modules.find(function(m) { return m.id === l.module; });

  var html = '<article class="lesson">';
  html += '<div class="lesson-head">';
  html += '<a class="back" href="#/m/' + l.module + '">' + (mod ? CG.esc(mod.title) : '') + '</a>';
  if (stepNum) html += '<span class="step">第 ' + stepNum + ' 步</span>';
  html += '</div>';
  html += '<h1>' + CG.esc(l.title) + '</h1>';
  ...
  if (pathIdx > 0) {
    var prev = CG.path[pathIdx - 1];
    html += '<a class="nav-prev" href="#/l/' + prev + '">上一课</a>';
  }
  if (!P.isRead(id)) {
    html += '<button class="mark-read-btn" onclick="CG.markAndNext(\'' + id + '\')">标记已学</button>';
  } else {
    html += '<span class="read-done" onclick="CG.toggleRead(\'' + id + '\')" title="点击取消已学" style="cursor:pointer">✓ 已学</span>';
  }
  if (pathIdx < CG.path.length - 1) {
    var next = CG.path[pathIdx + 1];
...
CG.markAndNext = function(id) {
  var P = CG.progress();
  P.markRead(id);
  var idx = CG.path.indexOf(id);
  if (idx < CG.path.length - 1) {
    window.location.hash = '#/l/' + CG.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + CG.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + CG.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
...
/* 在当前页切换已学/未学,不跳转 */
CG.toggleRead = function(id) {
  var P = CG.progress();
  if (P.isRead(id)) P.markUnread(id); else P.markRead(id);
  CG.views.lesson(id);
  if (CG.renderShell) CG.renderShell();
};
