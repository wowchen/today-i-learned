...
(function() {
  ...
  function save(data) {
    ...
    if (CG.sync) CG.sync.schedulePush();
    if (CG.onProgressChange) CG.onProgressChange();
  }

  CG.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < CG.path.length; i++) {
          if (!this.isRead(CG.path[i])) return i;
        }
        return CG.path.length;
      },
