...
CG.registerLesson = function(data) {
  CG.lessons[data.id] = data;
  CG.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
...
CG.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < CG.searchIndex.length; i++) {
    var item = CG.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < CG.terms.length; t++) {
    var term = CG.terms[t];
    var ts = 0;
