/* GitHub 进度同步(可选):fine-grained PAT + 独立 private repo + Contents API。
   流程:启动时 pull→merge;数据变更后 debounce 推送;PUT 遇 409/422(sha 过期)
   则重新 pull→merge→重试。token 仅存本机 localStorage,不入站点代码仓库。
   注意:本站 progress 是工厂(CG.progress() 每次返回新对象),故同步时带括号调用。 */
(function () {
  'use strict';
  window.CG = window.CG || {};

  var CFG_KEY = 'cg.sync.v1';
  var cfg = loadCfg();
  var pushTimer = null;
  var busy = false;
  var lastSha = null;
  ...
  function apiUrl() {
    // cfg.repo 形如 "owner/cg-progress"
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/cg.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  function pull() {
    ...
    return fetch(apiUrl(), { headers: headers() }).then(function (res) {
      ...
      return res.json().then(function (j) {
        lastSha = j.sha;
        CG.progress().merge(JSON.parse(b64decodeUtf8(j.content)));
        return true;
      });
    });
  }
  ...
  function push(retry) {
    ...
    var body = {
      ...
      content: b64encodeUtf8(CG.progress().exportJson()),
  ...
  CG.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/cg.json').trim();
      saveCfg();
    },
