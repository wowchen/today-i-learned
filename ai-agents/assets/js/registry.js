...
AGT.registerLesson = function(data) {
  AGT.lessons[data.id] = data;
  AGT.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
AGT.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < AGT.searchIndex.length; i++) {
    var item = AGT.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < AGT.terms.length; t++) {
    var term = AGT.terms[t];
    var ts = 0;
