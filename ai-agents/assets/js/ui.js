...
AGT.render = function(html) {
  document.getElementById('app').innerHTML = html;
  AGT.bindTerms();
};

AGT.findTerm = function(id) {
  for (var i = 0; i < AGT.terms.length; i++) {
    if (AGT.terms[i].id === id) return AGT.terms[i];
  }
  return null;
};

AGT.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      AGT.showTermCard(id, this);
    });
  }
};

AGT.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = AGT.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + AGT.esc(term.name) + '</b> <span class="tc-en">' + AGT.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + AGT.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + AGT.esc(term.analogy) + '</div>' : '') +
