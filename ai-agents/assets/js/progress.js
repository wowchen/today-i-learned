...
(function() {
  ...
  function save(data) {
    ...
    if (AGT.sync) AGT.sync.schedulePush();
    if (AGT.onProgressChange) AGT.onProgressChange();
  }

  AGT.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < AGT.path.length; i++) {
          if (!this.isRead(AGT.path[i])) return i;
        }
        return AGT.path.length;
      },
