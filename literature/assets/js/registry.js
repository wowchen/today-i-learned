...
LIT.registerLesson = function(data) {
  LIT.lessons[data.id] = data;
  LIT.searchIndex.push({
    id: data.id,
    type: 'lesson',
    title: data.title,
    module: data.module,
...
LIT.search = function(query) {
  if (!query || query.length < 1) return [];
  var q = query.toLowerCase();
  var results = [];

  for (var i = 0; i < LIT.searchIndex.length; i++) {
    var item = LIT.searchIndex[i];
    var score = 0;
  ...
  for (var t = 0; t < LIT.terms.length; t++) {
    var term = LIT.terms[t];
    var ts = 0;
