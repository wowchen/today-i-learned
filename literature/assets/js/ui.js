...
LIT.render = function(html) {
  document.getElementById('app').innerHTML = html;
  LIT.bindTerms();
};

LIT.findTerm = function(id) {
  for (var i = 0; i < LIT.terms.length; i++) {
    if (LIT.terms[i].id === id) return LIT.terms[i];
  }
  return null;
};

LIT.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      LIT.showTermCard(id, this);
    });
  }
};

LIT.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = LIT.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + LIT.esc(term.name) + '</b> <span class="tc-en">' + LIT.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + LIT.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + LIT.esc(term.analogy) + '</div>' : '') +
