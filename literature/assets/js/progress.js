...
(function() {
  ...
  function save(data) {
    ...
    if (LIT.sync) LIT.sync.schedulePush();
    if (LIT.onProgressChange) LIT.onProgressChange();
  }

  LIT.progress = function() {
    var data = load();
    return {
      ...
      currentStep: function() {
        for (var i = 0; i < LIT.path.length; i++) {
          if (!this.isRead(LIT.path[i])) return i;
        }
        return LIT.path.length;
      },
