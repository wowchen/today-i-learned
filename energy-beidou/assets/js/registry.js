/* 内容注册中心:内容即数据(ADR-3)。
   ...
   EBD.registerModule / EBD.registerPath / EBD.registerLesson / EBD.registerTerms
...
(function () {
  ...
  EBD.registerModule = function (m) {
    if (!m || !m.id) return;
    if (!modules[m.id]) moduleOrder.push(m.id);
    modules[m.id] = m;
  };

  EBD.registerPath = function (list) { path = list || []; };

  EBD.registerLesson = function (l) {
    if (!l || !l.id || !l.module) return;
    lessons[l.id] = l;
  };

  /* 术语库:数组批量注册,可多次调用(分模块扩充) */
  EBD.registerTerms = function (list) {
