...
(function () {
  ...
  /* ── 首页 ── */
  EBD.views.home = function () {
    var next = EBD.nextStep();
    var hero;
    if (next) {
      ...
      hero =
        '<section class="hero">' +
        '<div class="main">' +
        '<p class="eyebrow"><b>第 ' + (next.index + 1) + ' 步</b>' + EBD.esc(mod.title) + '</p>' +
        '<h1><mark>' + EBD.esc(next.step.title) + '</mark></h1>' +
        '<p class="intro">约 ' + (R().lesson(next.step.id).minutes || 5) +
        ' 分钟一课。不用想"接下来学什么",跟着路线走就行。</p>' +
        '<div class="cta">' +
        '<a class="btn solid" href="#/l/' + next.step.id + '">继续学习 第 ' + (next.index + 1) + ' 步 ' + EBD.icon('arrowR', 13) + '</a>' +
    ...
    var demo =
      ...
      '电力最离不开北斗的,其实是<gd data-term="shoushi">授时</gd>——让全网设备对准同一块表;' +
      '深山没信号的设施,靠<gd data-term="duanbaowen">短报文</gd>把数据传出来;' +
      '杆塔、大坝则用<gd data-term="xingbianjiance">形变监测</gd>盯着毫米级的位移;' +
      '要厘米级定位,就上<gd data-term="rtk">RTK</gd>。' +
      '</p><p class="sub" style="margin:10px 0 0">提示:点术语弹出卡片后,可以「收藏」或「去那一课」。</p></div>';

    var toc =
      '<div class="chapter" id="toc"><span>目录</span><h2>12 个模块</h2></div>' +
    ...
    EBD.render(hero + demo + toc + review);
  };

  function sidenote() {
    return '<aside class="sidenote">' +
      '<p><b>' + P().streak() + ' 天</b>连续学习</p>' +
      '<p><b>' + EBD.pathDone() + ' / ' + R().path().length + '</b>路线进度</p>' +
  ...
  function tocRow(m) {
    ...
    return '<li><span class="no">' + m.no + '</span>' +
      '<a class="t" href="#/m/' + m.id + '">' + EBD.esc(m.title) +
      '<small>' + EBD.esc(m.sub) + '</small></a>' +
      '<span class="lead"></span>' + pg + '</li>';
  }

  /* ── 完整路线 ── */
  EBD.views.path = function () {
    var next = EBD.nextStep();
    ...
    EBD.render(
      ...
      '<p class="sub">已完成 ' + EBD.pathDone() + ' / ' + R().path().length +
  ...
  EBD.views.module = function (id) {
    ...
    var coming = R().path().filter(function (s) {
      ...
      return '<li><span class="no">·</span><span class="t dead">' +
        EBD.esc(s.title) + '</span><span class="lead"></span><span class="pg">待编写</span></li>';
    }).join('');
    EBD.render(
      '<div class="crumb" style="margin-top:14px"><b>模块 ' + m.no + '</b> / ' + EBD.esc(m.sub) + '</div>' +
      '<h1 class="page">' + EBD.esc(m.title) + '</h1>' +
      '<p class="meta" style="font-family:inherit;font-size:14px;max-width:44em">' + EBD.esc(m.desc) + '</p>' +
