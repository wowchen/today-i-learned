...
(function () {
  ...
  function save() {
    ...
    if (EBD.sync) EBD.sync.schedulePush();
    if (EBD.onProgressChange) EBD.onProgressChange();
  }
  ...
  EBD.progress = {
    ...
    readCountOf: function (moduleId) {
      var n = 0;
      EBD.registry.lessonsOf(moduleId).forEach(function (l) {
        if (EBD.progress.isRead(l.id)) n++;
      });
      return n;
    },
