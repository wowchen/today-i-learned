/* 慢病管理站 · sync —— GitHub 进度同步(可选),与全站规范实现一致
   token 仅存本机 localStorage['SLP.sync.v1'],默认路径 progress/slp.json */
(function(){
  var NS='SLP', CFG_KEY=NS+'.sync.v1';
  var DEBOUNCE=4000, timer=null, cfg=null;

  function readCfg(){
    if(cfg) return cfg;
    try{ cfg=JSON.parse(localStorage.getItem(CFG_KEY)||'null')||{repo:'',branch:'main',token:'',path:'progress/slp.json',auto:true}; }
    catch(e){ cfg={repo:'',branch:'main',token:'',path:'progress/slp.json',auto:true}; }
    if(!cfg.path) cfg.path='progress/slp.json';
    return cfg;
  }
  ...
  async function push(){
    ...
    try{
      ...
      var body=btoa(unescape(encodeURIComponent(SLP.progress.exportJson())));
      var pr=await api('https://api.github.com/repos/'+c.repo+'/contents/'+c.path,{
        method:'PUT',
        body:JSON.stringify({message:'chore(slp): sync progress',content:body,branch:c.branch||'main',sha:sha})
      });
  ...
  SLP.sync={
    ...
    save:function(repo,branch,token,path,auto){
      ...
      c.token=(token||'').trim(); c.path=(path||'').trim()||'progress/slp.json'; c.auto=!!auto;
