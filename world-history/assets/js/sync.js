...
(function () {
  ...
  function apiUrl() {
    // cfg.repo 形如 "owner/whs-progress"
    return 'https://api.github.com/repos/' + cfg.repo + '/contents/' +
      (cfg.path || 'progress/whs.json') + '?ref=' + (cfg.branch || 'main');
  }
  function headers() {
    return {
  ...
  function pull() {
    ...
    return fetch(apiUrl(), { headers: headers() }).then(function (res) {
      ...
      return res.json().then(function (j) {
        lastSha = j.sha;
        WHS.progress().merge(JSON.parse(b64decodeUtf8(j.content)));
        return true;
      });
    });
  }
  ...
  function push(retry) {
    ...
    var body = {
      ...
      content: b64encodeUtf8(WHS.progress().exportJson()),
  ...
  WHS.sync = {
    ...
    setConfig: function (c) {
      ...
      cfg.path = (c.path || 'progress/whs.json').trim();
      saveCfg();
    },
