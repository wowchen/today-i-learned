...
WHS.render = function(html) {
  document.getElementById('app').innerHTML = html;
  WHS.bindTerms();
};

WHS.findTerm = function(id) {
  for (var i = 0; i < WHS.terms.length; i++) {
    if (WHS.terms[i].id === id) return WHS.terms[i];
  }
  return null;
};

WHS.bindTerms = function() {
  var els = document.querySelectorAll('gd[data-term]');
  for (var i = 0; i < els.length; i++) {
    els[i].addEventListener('click', function(e) {
      var id = this.getAttribute('data-term');
      WHS.showTermCard(id, this);
    });
  }
};

WHS.showTermCard = function(id, anchor) {
  var old = document.getElementById('term-popover');
  if (old) old.remove();

  var term = WHS.findTerm(id);
  if (!term) return;
  ...
  card.innerHTML =
    '<div class="tc-head"><b>' + WHS.esc(term.name) + '</b> <span class="tc-en">' + WHS.esc(term.en) + '</span></div>' +
    '<div class="tc-def">' + WHS.esc(term.def) + '</div>' +
    (term.analogy ? '<div class="tc-analogy">' + WHS.esc(term.analogy) + '</div>' : '') +
