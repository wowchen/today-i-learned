...
WHS.views.lesson = function(id) {
  var l = WHS.lessons[id];
  if (!l) {
    WHS.render('<div class="empty"><h2>课时待编写</h2><p>ID: ' + WHS.esc(id) + '</p><a href="#/">返回首页</a></div>');
    return;
  }
...
WHS.markAndNext = function(id) {
  var P = WHS.progress();
  P.markRead(id);
  var idx = WHS.path.indexOf(id);
  if (idx < WHS.path.length - 1) {
    window.location.hash = '#/l/' + WHS.path[idx + 1];
  } else {
...
function renderQuiz(q, idx, lessonId) {
  ...
  if (q.source) html += ' · ' + WHS.esc(q.source);
  html += '</div>';
  ...
  if (q.type === 'choice') {
    ...
    for (var i = 0; i < q.options.length; i++) {
      html += '<button class="opt" data-answer="' + q.answer + '" data-idx="' + i + '">' +
              labels[i] + '. ' + WHS.esc(q.options[i]) + '</button>';
    }
    html += '</div>';
  } else if (q.type === 'fill') {
